...
class PhantomMultiWallet {
  ...
  getPhantomProvider() {
    ...
    // Add checks for EVM capabilities to better handle parameter validation
    if (window.phantom?.ethereum) {
      // Tag the provider with EVM support so we can handle it differently
      provider._hasEVMSupport = true;

      // Store a reference to the ethereum provider for potential use
      provider._evmProvider = window.phantom.ethereum;
    }
  ...
  // Connect to a new Phantom wallet
  async connectWallet() {
    try {
      ...
      try {
        ...
        shouldConnect = !currentWallet;
      } catch (e) {
        console.log("Error checking current wallet:", e);
        // We'll proceed with connection anyway
      }
      ...
      if (shouldConnect) {
        ...
        while (retries < maxRetries && !response) {
          try {
            // Fix for "Missing or invalid parameters" error:
            // Ensure we pass a valid connect options object with proper parameters
            const connectOptions = { onlyIfTrusted: false };

            // Additional validation for EVM support
            // Check if we need to specify chain for EVM compatibility
            if (provider._hasEVMSupport) {
              console.log("Detected Phantom with EVM support - using appropriate parameters");
              // For EVM-supporting wallets, we need to ensure compatibility
              // This prevents the "Missing or invalid parameters" error in evmAsk.js
              connectOptions.chainId = 'solana:mainnet';
            }

            response = await provider.connect(connectOptions);
            break;
          } catch (err) {
            lastError = err;
            retries++;
            ...
            // Check for specific EVM error
            if (err.message && err.message.includes("Missing or invalid parameters")) {
              console.log("Detected EVM parameter error, retrying with modified parameters");
              try {
                // Try with a more specific approach for EVM
                if (provider._evmProvider) {
                  // Try connecting via the EVM provider directly
                  const evmResponse = await provider._evmProvider.request({
                    method: 'eth_requestAccounts'
                  });

                  // If we get here, we succeeded with the EVM connection
                  // Now try to get the Solana account
                  response = await provider.connect({ onlyIfTrusted: true });
                  break;
                }
              } catch (evmErr) {
                console.warn("EVM-specific connection also failed:", evmErr);
  ...
  async discoverAllWallets(force = false) {
    // Check if discovery is disabled (emergency fix for infinite loops)
    const isDisabled = localStorage.getItem('phantomDiscoveryDisabled') === 'true';

    if (isDisabled && !force) {
      console.log("EMERGENCY FIX: Wallet discovery is disabled to prevent infinite loops");

      // Return the existing discovered wallets
      try {
        const existingWallets = JSON.parse(localStorage.getItem('phantomDiscoveredWallets') || '[]');
        return existingWallets;
      } catch (e) {
        console.error("Error reading existing wallets:", e);
        return [];
      }
    }
    ...
    // Check if we recently ran a discovery to prevent looping
    const lastDiscoveryTime = parseInt(localStorage.getItem('phantomLastDiscoveryTime') || '0');
    const now = Date.now();

    // Don't run discovery more than once every 60 seconds unless forced
    if (!force && now - lastDiscoveryTime < 60000) {
      console.log("Skipping discovery - already ran within the last minute");
    ...
    localStorage.setItem('phantomLastDiscoveryTime', now.toString());
    ...
    if (currentAddress) {
      // No need to log this for every discovery - that's what's flooding the console
      if (force) {
        console.log("Current connected wallet:", currentAddress);
      }
    ...
    try {
      ...
      for (const address of storedWallets) {
        if (!discoveredWallets.has(address)) {
          ...
          // Only log if forced/manual discovery to reduce spam
          if (force) {
            console.log("Including previously discovered wallet:", address);
          }
        }
      }
    } catch (e) {
    ...
    const walletList = [...discoveredWallets];
    localStorage.setItem('phantomDiscoveredWallets', JSON.stringify(walletList));
  ...
  try {
    ...
    if (walletState[targetAddress]) {
      ...
      try {
        ...
        // Try to reconnect with the wallet's identifier
        // Create proper connect options to avoid parameter errors
        const connectOptions = { onlyIfTrusted: false };

        // Add EVM-specific options if needed
        if (provider._hasEVMSupport) {
          connectOptions.chainId = 'solana:mainnet';
        }
        ...
        try {
          ...
          // If we get a parameter error and have EVM support, try the EVM approach
          if (connErr.message && connErr.message.includes("Missing or invalid parameters") && provider._evmProvider) {
            console.log("Trying EVM-specific connection approach");

            // First connect via EVM to get user attention
            await provider._evmProvider.request({
              method: 'eth_requestAccounts'
            });

            // Then try connecting to Solana side
            await provider.connect({ onlyIfTrusted: true });
          } else {
            throw connErr; // Rethrow if not a parameter error or no EVM support
          }
        }
