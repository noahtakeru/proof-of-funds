...
/**
 * standardize-api-files.js - API-related file standardizer
 * 
 * This script standardizes the API-related files based on the analysis
 * from check-api-files.js
 */
...
const FILES_TO_STANDARDIZE = [
  ...
  { path: 'pages/api/verify-transaction.js', action: 'convert-to-esm', newPath: 'pages/api/verify-transaction.mjs' },
  { path: 'pages/api/zk/status.js', action: 'convert-to-esm', newPath: 'pages/api/zk/status.mjs' },
  { path: 'pages/api/zk/verificationKey.js', action: 'convert-to-esm', newPath: 'pages/api/zk/verificationKey.mjs' },
  { path: 'pages/api/zk/verify.js', action: 'convert-to-esm', newPath: 'pages/api/zk/verify.mjs' },
  
  // CommonJS files with incorrect extension (JS to CJS)
  { path: 'lib/zk/src/zkProxyClient.js', action: 'convert-to-cjs', newPath: 'lib/zk/src/zkProxyClient.cjs' },
  
  // Mixed/Dual format - leave as is
  { path: 'pages/api/zk/fullProve.js', action: 'leave-as-is' }
];
...
async function fixFile(filePath, targetPath, moduleFormat) {
  try {
    ...
    const fullTargetPath = path.join(PROJECT_ROOT, targetPath);
...
// Main execution
async function main() {
  console.log(`Starting API files standardization${DRY_RUN ? ' (dry run)' : ''}...`);
