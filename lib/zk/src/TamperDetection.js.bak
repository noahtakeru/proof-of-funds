...
class TamperDetection {
   ...
   */
  async verify(data, key) {
    const operationId = `verify_${Date.now()}`;

    try {
      // Validate the data structure
      if (!data || typeof data !== 'object') {
        throw new InputError('Invalid data format for verification', {
          code: ErrorCode.INPUT_VALIDATION_FAILED,
          operationId,
      ...
      if (!data.signature || !data.canaries || !data.meta) {
        throw new InputError('Data is missing required tamper detection fields', {
          code: ErrorCode.INPUT_VALIDATION_FAILED,
          operationId,
          details: { 
            hasSignature: !!data.signature,
            hasCanaries: !!data.canaries,
            hasMeta: !!data.meta
          }
        });
      }
      ...
      // Store signature and remove it from data (since it wasn't part of the original signing)
      const signature = dataToVerify.signature;
      delete dataToVerify.signature;
      
      // Check canary values
      const canariesValid = await this.verifyCanaries(dataToVerify.canaries, key);
      if (!canariesValid) {
        return false; // Tampering detected in canary values
      }
      
      // Calculate expected signature
      const expectedSignature = await this.calculateSignature(dataToVerify, key);
  ...
  async signWithFallback(data, privateKey) {
    try {
      ...
      try {
        const crypto = await this.getCryptoImplementation();
        ...
        if (crypto.subtle) {
          ...
          const cryptoNode = cryptoModule.default;
   ...
   * @returns {Promise<Object>} Crypto implementation object
   */
  async getCryptoImplementation() {
    if (typeof window !== 'undefined' && window.crypto) {
      ...
      try {
        const cryptoModule = await import('crypto');
        return { node: cryptoModule.default };
      } catch (e) {
    ...
    try {
      ...
      if (crypto.subtle) {
        ...
        try {
          ...
          throw new CryptoError(`Node.js high isolation verification failed: ${nodeCryptoError.message}`, {
            code: ErrorCode.CRYPTO_VERIFICATION_FAILED,
            details: { cause: nodeCryptoError.message },
            cause: nodeCryptoError
          });
        }
      }
    } catch (error) {
  ...
  /**
   * Signs a message for remote verification
   * @param {Object|string} message - Message to sign
   * @param {string} key - Secret key for signing
   * @returns {Promise<string>} Signature
   */
  async signForRemote(message, key) {
    const operationId = 'TamperDetection:signForRemote';
    
    try {
      // Input validation
      if (!message) {
        throw new InputError('Missing message for remote signing', {
          code: ErrorCode.INPUT_MISSING_REQUIRED,
          operationId,
          userFixable: true,
          recoverable: true,
          details: { messageType: typeof message }
        });
      }
      ...
      // Create a signature using HMAC or similar
      if (this.subtle) {
        try {
          // Import the key
          const cryptoKey = await this.subtle.importKey(
          ...
          // Log the fallback
          zkErrorLogger.log('WARNING', 'Web Crypto signing failed, using fallback', {
            operationId,
            details: { cause: cryptoError.message },
            recoverable: true
          });
      ...
      return this.fallbackSigningMethod(messageStr, key);
    } catch (error) {
      // Log error
      zkErrorLogger.logError(error, {
        context: 'Remote message signing',
        operationId,
  ...
  /**
   * Verifies a remotely signed message
   * @param {Object|string} message - Message that was signed
   * @param {string} signature - Signature to verify
   * @param {string} key - Key used for signing
   * @returns {Promise<boolean>} Whether the signature is valid
   */
  async verifyRemoteSignature(message, signature, key) {
    const operationId = 'TamperDetection:verifyRemoteSignature';
    
    try {
      // Input validation
      if (!message) {
        throw new InputError('Missing message for signature verification', {
          code: ErrorCode.INPUT_MISSING_REQUIRED,
          operationId,
          userFixable: true,
          recoverable: true,
      ...
      if (!signature) {
        throw new InputError('Missing signature for verification', {
          code: ErrorCode.INPUT_MISSING_REQUIRED,
          operationId,
          userFixable: true,
          recoverable: true,
          details: { signatureType: typeof signature }
        });
      }
      
      if (!key) {
        throw new InputError('Missing key for signature verification', {
          code: ErrorCode.INPUT_MISSING_REQUIRED,
          operationId,
          userFixable: true,
          recoverable: true,
      ...
      // Verify using Web Crypto if available
      if (this.subtle) {
        try {
          // Import the key
          const cryptoKey = await this.subtle.importKey(
          ...
          // Log the fallback
          zkErrorLogger.log('WARNING', 'Web Crypto verification failed, using fallback', {
            operationId,
            details: { cause: cryptoError.message },
            recoverable: true
          });
      ...
      return this.fallbackVerificationMethod(messageStr, signature, key);
    } catch (error) {
      // Log error
      zkErrorLogger.logError(error, {
        context: 'Remote signature verification',
        operationId,
        messageType: typeof message,
        signatureType: typeof signature
      });
      
      // For security, default to false for any errors
      return false;
    }
  }
  
  /**
   * Fallback signing method for environments without Web Crypto
   * @param {string} message - Message to sign
   * @param {string} key - Key to sign with
   * @returns {string} Signature
   * @private
   */
  fallbackSigningMethod(message, key) {
    // Simple HMAC-like implementation
    let result = '';
    const keyLen = key.length;
    
    // Create a simple message digest
    for (let i = 0; i < message.length; i++) {
      const charCode = message.charCodeAt(i);
      const keyChar = key.charCodeAt(i % keyLen);
      result += ((charCode ^ keyChar) % 16).toString(16);
    }
    
    // Add some randomness
    result += this.generateRandomValue();
    
    return result;
  }
  
  /**
   * Fallback verification method for environments without Web Crypto
   ...
   * @private
   */
  fallbackVerificationMethod(message, signature, key) {
    // Generate a new signature and compare
    const expectedSignature = this.fallbackSigningMethod(message, key);
    ...
    // If they don't match exactly, verify the first part (excluding the random part)
    // Since our fallback method adds random data at the end
    const minLength = Math.min(signature.length, expectedSignature.length, message.length);
    const sigPart = signature.substring(0, minLength);
    const expectedPart = expectedSignature.substring(0, minLength);
    
    return sigPart === expectedPart;
  }
  ...
  hexToUint8Array(hex) {
    ...
    for (let i = 0; i < paddedHex.length; i += 2) {
      byteArray[i / 2] = parseInt(paddedHex.substring(i, i + 2), 16);
    }
