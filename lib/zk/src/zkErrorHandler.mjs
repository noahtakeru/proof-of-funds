...
function getErrorLogger() {
  if (!_zkErrorLogger) {
    // In Node.js environment, we can import dynamically
    if (typeof require !== 'undefined') {
      try {
        const { zkErrorLogger } = require('./zkErrorLogger.js');
        _zkErrorLogger = zkErrorLogger;
      } catch (err) {
        // Create a minimal fallback logger if import fails
        _zkErrorLogger = {
          logError: (error, context = {}) => {
            console.error('[ZK Fallback Error Logger]', error.message || 'Unknown error', {
              error,
              context
            });
        ...
        console.warn(`Using fallback error logger: ${err.message}`);
      }
    } else {
      // In browser, create a minimal fallback logger
      _zkErrorLogger = {
        logError: (error, context = {}) => {
          console.error('[ZK Fallback Error Logger]', error.message || 'Unknown error', {
            error,
      ...
      console.warn('Using fallback error logger in browser environment');
    }
  }
...
export {
  ...
  createZKError,
  ZKErrorFactory,
  getErrorLogger // Export the getErrorLogger function
};
