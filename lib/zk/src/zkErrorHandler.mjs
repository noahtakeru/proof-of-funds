  ...
  if (!_zkErrorLogger) {
    ...
    if (typeof require !== 'undefined') {
      try {
        ...
        // Throw a clear error if logger import fails
        throw new Error(`Failed to load error logger: ${err.message}`);
      }
    } else {
      // In browser, we should have already imported the logger in index.mjs
      // If we got here, it means there's a programming error that needs fixing
      throw new Error('Error logger not initialized in browser environment. This should never happen and indicates a coding error.');
    }
  }
