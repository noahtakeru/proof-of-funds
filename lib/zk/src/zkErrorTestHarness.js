...
let errorHandler, errorLogger, zkRecovery;
...
const modulesPromise = (async function loadModules() {
  try {
    if (isEsm) {
      // ESM environment - this will never execute in this file
      // but keeps the pattern consistent with other modules
      const [handler, logger, recovery] = await Promise.all([
        import('./zkErrorHandler.mjs'),
        import('./zkErrorLogger.mjs'),
        import('./zkRecoverySystem.mjs')
      ]);
      
      errorHandler = handler;
      errorLogger = logger.zkErrorLogger;
      zkRecovery = recovery.zkRecovery;
    } else {
      ...
      zkRecovery = require('./zkRecoverySystem.js');
    }
  } catch (err) {
    console.error(`Error in zkErrorTestHarness module initialization: ${err.message}`);
    throw new ModuleLoadError(
      `Failed to initialize error test harness module: ${err.message}`,
      { environment: 'CommonJS', moduleType: 'test harness' },
      err
    );
  }
})();

// Destructure commonly used items for convenience once modules are loaded
let ZKError, ErrorCode, ErrorSeverity, ErrorCategory,
    CircuitError, ProofError, VerificationError,
    MemoryError, NetworkError, SecurityError, isZKError;
...
modulesPromise.then(() => {
  ...
  console.error('Failed to initialize zkErrorTestHarness module:', err);
});
...
/**
 * Wait for all module dependencies to be loaded before using these functions
 * @private
 */
const awaitModuleInitialization = async () => {
  await modulesPromise;
};

// Create properly exported API functions with module initialization check
const createApiFunction = (fn) => {
  return async (...args) => {
    await awaitModuleInitialization();
    return fn(...args);
  };
};

// Create properly wrapped API exports
const api = {
  withNetworkFailureSimulation: createApiFunction(withNetworkFailureSimulation),
  withMemoryConstraintSimulation: createApiFunction(withMemoryConstraintSimulation),
  withCorruptedInputSimulation: createApiFunction(withCorruptedInputSimulation),
  withWorkerTerminationSimulation: createApiFunction(withWorkerTerminationSimulation),
  batchRecoveryTest: createApiFunction(batchRecoveryTest),
  createErrorPropagationTest: createApiFunction(createErrorPropagationTest)
};
  ...
  try {
    ...
    if (errorLogger && errorLogger.logError) {
      ...
      await errorLogger.logError(error, {
        context: context.context || 'zkErrorTestHarness.js',
        ...context
      });
    } else {
...
// Export all test functions using CommonJS exports
module.exports = api;