/**
 * Memory Efficient Cache Implementation for ZK Operations
 * 
 * This module provides an optimized caching system designed specifically for ZK operations,
 * with features such as memory-efficient storage, configurable eviction policies,
 * automatic compression, and resilience against memory constraints.
 * 
 * Key features:
...
export enum EvictionPolicy {
    /** Least Recently Used - removes items that haven't been accessed for the longest time */
    LRU = 'lru',
    /** Least Frequently Used - removes items that are accessed least often */
    LFU = 'lfu',
    /** First In, First Out - removes oldest items first */
    FIFO = 'fifo',
    /** Priority-based - removes items with lowest priority first */
    PRIORITY = 'priority'
}
...
export interface CacheConfig {
    ...
    /** Size threshold in bytes for compressing values */
    compressionThreshold?: number;
    /** Callback function when memory pressure exceeds 90% */
    onMemoryPressure?: (percentUsed: number) => void;
}
...
interface CacheEntry {
    ...
    expiresAt: number;
    /** Size of the value in bytes (estimated) */
    size: number;
    /** Optional priority value (higher = more important) */
    priority?: number;
...
export class MemoryEfficientCache {
    ...
    private onMemoryPressure?: (percentUsed: number) => void;
        ...
        try {
            ...
            if (this.totalSize + valueSize > this.maxSize) {
                zkErrorLogger.log('WARNING', 'Cache entry too large to store', {
                    ...
                    size: valueSize,
                    maxSize: this.maxSize,
                    remainingSpace: this.maxSize - this.totalSize
                });
                return false;
            }
     ...
     */
    invalidate(keyOrPattern: string, isPattern: boolean = false): number {
        try {
            if (!isPattern) {
     ...
     */
    private evictEntries(bytesNeeded: number): void {
        if (this.cache.size === 0 || bytesNeeded <= 0) {
            return;
        }

        let bytesFreed = 0;
        const sortedEntries: Array<[string, CacheEntry]> = Array.from(this.cache.entries());
