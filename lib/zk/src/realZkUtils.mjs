 ...
 * This avoids mixing CommonJS requires in ESM modules
 */
const mockFsPromises = {
  readFile: () => Promise.reject(new Error('File system not available in ESM/browser environment')),
  writeFile: () => Promise.reject(new Error('File system not available in ESM/browser environment')),
  access: () => Promise.reject(new Error('File system not available in ESM/browser environment'))
};

/**
 * Get filesystem promises in a way that doesn't mix module systems
 * This avoids the "ESM file contains CommonJS requires" warning
 */
function getFsPromises() {
  ...
  // In Node.js environments, use dynamic import for ESM compatibility
  // Since this is conditional and only runs on the server, it won't affect
  // webpack builds in the browser
  try {
    ...
    if (globalThis.process && globalThis.process.versions && globalThis.process.versions.node) {
      // Since we can't use require in an ESM module, just return the mock
      // The actual implementation will only be used on the server
      return mockFsPromises;
    }
  } catch (e) {
    // Failure means we're not in Node.js
  }
 ...
 * @example
 * // In CommonJS:
 * const realZkUtils = require('./realZkUtils.cjs');
