...
/**
 * Custom error class for GCP Secret Manager operations
 * @extends ZKError
 */
class GCPSecretError extends ZKError {
    /**
     * Create a GCP Secret Manager error
    ...
    constructor(code, message, details = {}) {
        ...
        this.name = 'GCPSecretError';
    }
}

/**
 * Google Cloud Platform Secret Manager client implementation
 * Provides secure storage for cryptographic keys and secrets with local fallback capability
 * @class
 */
export class GCPSecretManager {
    ...
    async createLocalSecret(params) {
        try {
            ...
            if (!this.fallbackToLocal) {
                throw new GCPSecretError('SEC-GCP-LOCAL-001', 'Local fallback not enabled', 
                    { operation: 'createLocalSecret' });
            }
    ...
    async getLocalSecret(params) {
        try {
            ...
            const secretDir = path.join(this.localFallbackDir, params.secretId);
            if (!fs.existsSync(secretDir)) {
                throw new GCPSecretError('SEC-GCP-LOCAL-002', `Secret ${params.secretId} not found`, 
                    { secretId: params.secretId, operation: 'getLocalSecret' });
            }

            const metadataPath = path.join(secretDir, 'metadata.json');
            if (!fs.existsSync(metadataPath)) {
                throw new GCPSecretError('SEC-GCP-LOCAL-003', `Secret ${params.secretId} metadata not found`, 
                    { secretId: params.secretId, operation: 'getLocalSecret' });
            }
            ...
            if (!fs.existsSync(versionPath)) {
                throw new GCPSecretError('SEC-GCP-LOCAL-004', `Secret ${params.secretId} version ${versionId} not found`, 
                    { secretId: params.secretId, versionId, operation: 'getLocalSecret' });
            }
    ...
    async ensureInitialized() {
        if (!this.isInitialized) {
            ...
            if (!success && !this.fallbackToLocal) {
                throw new GCPSecretError('SEC-GCP-INIT-001', 'GCP Secret Manager initialization failed and local fallback is disabled', 
                { operationId: this.operationId });
            }
        }
    }
