...
// Try importing ethers from the test setup if available, or use regular ethers
let ethers;
try {
  const setup = await import('./__tests__/setup.js');
  ethers = setup.mockEthers;
} catch (e) {
  try {
    ethers = await import('ethers');
  } catch (err) {
    console.warn('Could not import ethers, using placeholder implementation');
    // Simplified ethers implementation for testing
    ethers = {
      utils: {
