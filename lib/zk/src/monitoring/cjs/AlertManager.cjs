 ...
 * @property {string} type - One of the NotificationChannelType values
...
class AlertManager extends EventEmitter {
   ...
   * @param {string} alert.metricName - Name of the metric
   * @param {string} alert.severity - Severity level (INFO, WARNING, ERROR, CRITICAL)
   * @param {number} alert.value - Current value that triggered the alert
   * @param {number} alert.threshold - Threshold value
   * @param {string} alert.message - Alert message
   * @param {string} [alert.source] - Source of the alert
   * @param {string[]} [alert.tags] - Tags for the alert
   * @returns {string|null} The created alert ID or null if creation failed
   */
  createAlert(alert) {
    try {
   ...
   * @param {number} [options.toLevel] - Target escalation level
   * @param {string} [options.escalatedBy] - Who escalated the alert
   * @param {string} [options.reason] - Reason for escalation
   * @param {string[]} [options.notificationChannels] - Channels to notify
   * @returns {boolean} True if the alert was successfully escalated
   */
  escalateAlert(alertId, options = {}) {
    try {
      // Check if alert exists and is not already resolved
      if (!this.trackedAlerts.has(alertId)) {
        zkErrorLogger.log('WARNING', `Alert does not exist: ${alertId}`, {
