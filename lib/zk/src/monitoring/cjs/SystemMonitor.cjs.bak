...
class SystemMonitor extends EventEmitter {
   ...
   * @param {boolean} [options.aggregate] - Whether to aggregate data for histograms/summaries
   * @param {boolean} [options.checkThresholds] - Whether to check against defined thresholds
   * @returns {boolean} True if the metric was tracked successfully
   */
  trackMetric(metricName, value, labels = {}, options = {}) {
    try {
   ...
   * @param {Object} [options.timeRange] - Time range options
   * @param {Date} [options.timeRange.start] - Start time
   * @param {Date} [options.timeRange.end] - End time
   * @param {number} [options.timeRange.durationMs] - Duration in milliseconds
   * @param {string} [options.aggregation] - Aggregation function ('avg', 'sum', 'min', 'max', 'count', 'p95')
   * @param {string} [options.interval] - Grouping interval ('minute', 'hour', 'day')
   * @param {Object} [options.labelFilters] - Label filters
   * @param {number} [options.limit] - Maximum number of data points to return
   * @returns {Array|null} Array of metric data points or null if an error occurred
   */
  getMetricHistory(metricName, options = {}) {
    try {
      ...
      let startTime;
      let endTime = options.timeRange && options.timeRange.end ? options.timeRange.end : new Date();
      
      if (options.timeRange && options.timeRange.start) {
        startTime = options.timeRange.start;
      } else if (options.timeRange && options.timeRange.durationMs) {
      ...
      if (options.aggregation && filteredData.length > 0) {
        // Group by interval
        const groupedByInterval = new Map();
   ...
   * @param {string} threshold.operator - Comparison operator ('>', '<', '>=', '<=', '==', '!=')
   * @param {number} threshold.value - Threshold value
   * @param {number} [threshold.duration] - Duration in ms the condition must be true
   * @param {string} [threshold.severity] - Alert severity
   * @param {string} [threshold.description] - Alert description
   * @param {Object} [threshold.labels] - Label filters
   * @param {string[]} [threshold.notificationChannels] - Notification channels
   * @returns {string|null} The ID of the created alert or null if an error occurred
   */
  setThresholdAlert(metricName, threshold) {
    try {
      // Check if metric exists
      if (!this.metrics.has(metricName)) {
