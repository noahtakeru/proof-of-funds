...
// Standalone implementation of getCircuitMemoryRequirements to break circular dependencies
export function getCircuitMemoryRequirements(circuitType, operationType = 'proving') {
  // Default memory requirements (in MB) for different circuit types
  const memoryRequirements = {
    standard: {
      proving: 1024, // 1GB for standard proof generation
      verification: 128 // 128MB for verification
    },
    threshold: {
      proving: 1536, // 1.5GB for threshold proof generation
      verification: 192 // 192MB for verification
    },
    maximum: {
      proving: 2048, // 2GB for maximum proof generation
      verification: 256 // 256MB for verification
    }
  };

  // Normalize circuit type
  const normalizedType = circuitType ? circuitType.toLowerCase() : 'standard';
  
  // Get requirements for the circuit type, defaulting to standard if not found
  const requirements = memoryRequirements[normalizedType] || memoryRequirements.standard;
  
  // Return the requirements for the requested operation, defaulting to proving
  return {
    [operationType]: requirements[operationType] || requirements.proving,
    minMemory: requirements[operationType] || requirements.proving,
    recommended: requirements[operationType] * 1.5, // 50% buffer for best performance
    proving: requirements.proving,
    verification: requirements.verification
  };
}

// For static ESM imports, export a default object with the bridge function
export default {
