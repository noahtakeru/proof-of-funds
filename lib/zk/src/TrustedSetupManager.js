...
// Import error handling classes for proper error handling
import { 
  SystemError, 
  SecurityError, 
  InputError, 
...
// Import error logger for consistent error logging
import errorLoggerModule from './zkErrorLogger.mjs';
const { zkErrorLogger } = errorLoggerModule;
...
class TrustedSetupManager {
  ...
  initializeCeremony(params) {
    const operationId = `init_ceremony_${Date.now()}`;
    ...
    try {
      ...
      if (!circuitId || !circuitName) {
        throw new InputError('Circuit ID and name are required', {
          code: ErrorCode.INPUT_MISSING_REQUIRED,
          operationId,
          recoverable: true,
          userFixable: true,
          details: { providedParams: Object.keys(params) }
        });
      }
      ...
      if (existingCeremonies.length > 0) {
        ...
        throw new InputError(`An active ceremony for circuit ${circuitId} already exists (${activeId})`, {
      ...
      if (initialParameters) {
        ...
        if (typeof initialParameters !== 'object') {
          throw new InputError('Initial parameters must be a valid object', {
            code: ErrorCode.INPUT_TYPE_ERROR,
            operationId,
            recoverable: true,
            userFixable: true,
            details: {
              providedType: typeof initialParameters,
              expectedType: 'object'
            }
          });
        }
      ...
      // Log error using both audit logger and zkErrorLogger
      ...
      // If it's already a ZKError, log it through zkErrorLogger and re-throw
      if (isZKError(error)) {
        zkErrorLogger.logError(error, { 
          context: 'TrustedSetupManager.initializeCeremony',
          circuitId: params.circuitId || 'unknown'
        });
        throw error;
      }
      
      // Otherwise, wrap in SystemError
      const systemError = new SystemError(`Ceremony initialization failed: ${error.message}`, {
  ...
  registerParticipant(ceremonyId, participant) {
    const operationId = `register_participant_${Date.now()}`;
    
    try {
      // Validate ceremony exists
      const ceremony = this.ceremonies.get(ceremonyId);
      if (!ceremony) {
        throw new InputError(`Ceremony ${ceremonyId} not found`, {
          code: ErrorCode.INPUT_VALIDATION_FAILED,
          operationId,
          recoverable: false,
          userFixable: true,
      ...
      if (ceremony.status !== 'initialized' && ceremony.status !== 'in_progress') {
        throw new InputError(`Ceremony ${ceremonyId} is not accepting registrations (status: ${ceremony.status})`, {
          code: ErrorCode.INPUT_VALIDATION_FAILED,
          operationId,
          recoverable: false,
          userFixable: true,
          details: {
            ...
            validStatuses: ['initialized', 'in_progress']
          }
        });
      }
      ...
      if (!participant.id || !participant.publicKey) {
        throw new InputError('Participant ID and public key are required', {
          code: ErrorCode.INPUT_MISSING_REQUIRED,
          operationId,
          recoverable: true,
          userFixable: true,
          details: {
            hasId: !!participant.id,
            hasPublicKey: !!participant.publicKey
          }
        });
      }
      ...
      if (existingParticipant) {
        throw new InputError(`Participant ${participant.id} has already contributed to this ceremony`, {
          code: ErrorCode.INPUT_VALIDATION_FAILED,
          operationId,
          recoverable: false,
          userFixable: true,
      ...
      if (isZKError(error)) {
        zkErrorLogger.logError(error, { 
          context: 'TrustedSetupManager.registerParticipant',
          ceremonyId,
      ...
      const inputError = new InputError(`Failed to register participant: ${error.message}`, {
        code: ErrorCode.INPUT_VALIDATION_FAILED,
        operationId,
        recoverable: true,
        userFixable: true,
  ...
  async submitContribution(ceremonyId, contribution) {
    ...
    const operationId = `submit_contribution_${contributionStartTime}`;
    ...
    try {
      ...
      if (ceremony.status !== 'in_progress') {
        throw new InputError(`Ceremony ${ceremonyId} is not accepting contributions (status: ${ceremony.status})`, {
          code: ErrorCode.INPUT_VALIDATION_FAILED,
          operationId,
          recoverable: false,
          userFixable: true,
      ...
      if (!contribution.participantId || !contribution.parameters || !contribution.hash) {
        throw new InputError('Participant ID, parameters, and hash are required', {
          code: ErrorCode.INPUT_MISSING_REQUIRED,
          operationId,
          recoverable: true,
          userFixable: true,
          details: {
            hasParticipantId: !!contribution.participantId,
            hasParameters: !!contribution.parameters,
            hasHash: !!contribution.hash
          }
        });
      }
      ...
      if (contribution.entropy) {
        if (typeof contribution.entropy !== 'string' || 
          ...
          throw new InputError(`Entropy must be a string of at least ${this.config.entropyMinLength} characters`, {
            code: ErrorCode.INPUT_VALIDATION_FAILED,
            operationId,
            recoverable: true,
            userFixable: true,
            details: {
              providedEntropyType: typeof contribution.entropy,
              providedEntropyLength: typeof contribution.entropy === 'string' ? contribution.entropy.length : 0,
              requiredEntropyLength: this.config.entropyMinLength
            }
          });
        }
      }
      ...
      if (participantIndex === -1) {
        throw new InputError(`Participant ${contribution.participantId} is not registered for this ceremony`, {
          code: ErrorCode.INPUT_VALIDATION_FAILED,
          operationId,
          recoverable: false,
          userFixable: true,
      ...
      if (calculatedHash !== contribution.hash) {
        ...
        throw new SecurityError('Parameter hash verification failed', {
      ...
      if (!validationResult.isValid) {
        ...
        throw new SecurityError(`Parameter validation failed: ${validationResult.errors.join(', ')}`, {
          code: ErrorCode.SECURITY_DATA_INTEGRITY,
          operationId,
          recoverable: false,
          securityCritical: true,
      ...
      if (isZKError(error)) {
        zkErrorLogger.logError(error, { 
          context: 'TrustedSetupManager.submitContribution',
          ceremonyId,
          contributionId,
          participantId: contribution?.participantId || 'unknown',
          processingTimeMs: Date.now() - contributionStartTime
      ...
      // Otherwise, wrap in SystemError
      const systemError = new SystemError(`Contribution submission failed: ${error.message}`, {
  ...
  async verifyCeremony(ceremonyId, verification) {
    const operationId = `verify_ceremony_${Date.now()}`;
    ...
    try {
      ...
      if (!verification.verifierId || verification.result === undefined) {
        throw new InputError('Verifier ID and result are required', {
          code: ErrorCode.INPUT_MISSING_REQUIRED,
          operationId,
          recoverable: true,
          userFixable: true,
          details: {
            hasVerifierId: !!verification.verifierId,
            hasResult: verification.result !== undefined
          }
        });
      }
      ...
      if (existingVerification) {
        throw new InputError(`Verifier ${verification.verifierId} has already verified this ceremony`, {
          code: ErrorCode.INPUT_VALIDATION_FAILED,
          operationId,
          recoverable: false,
          userFixable: true,
      ...
      if (isZKError(error)) {
        zkErrorLogger.logError(error, { 
          context: 'TrustedSetupManager.verifyCeremony',
          ceremonyId,
          verifierId: verification?.verifierId || 'unknown'
        });
        throw error;
      }
      
      // Otherwise, wrap in InputError
      const inputError = new InputError(`Verification failed: ${error.message}`, {
        code: ErrorCode.INPUT_VALIDATION_FAILED,
        operationId,
        recoverable: true,
        userFixable: true,
  ...
  async finalizeCeremony(ceremonyId) {
    ...
    const operationId = `finalize_ceremony_${finalizationStartTime}`;
    ...
    try {
      ...
      if (!this.checkCeremonyCompletion(ceremony)) {
        throw new InputError(`Ceremony ${ceremonyId} is not ready for finalization`, {
          code: ErrorCode.INPUT_VALIDATION_FAILED,
          operationId,
          recoverable: false,
          userFixable: true,
          details: {
            ceremonyId,
            currentParticipants: ceremony.contributions.filter(c => c.status === 'contributed').length,
            requiredParticipants: ceremony.requiredParticipants
          }
        });
      }
      ...
      if (!validationResult.isValid) {
        ...
        throw new SecurityError(`Final parameter validation failed: ${validationResult.errors.join(', ')}`, {
          code: ErrorCode.SECURITY_DATA_INTEGRITY,
          operationId,
          recoverable: false,
          securityCritical: true,
      ...
      if (!keyValidationResult.isValid) {
        ...
        throw new SecurityError(`Verification key validation failed: ${
          keyValidationResult.errors?.join(', ') || keyValidationResult.error
        }`, {
          code: ErrorCode.SECURITY_DATA_INTEGRITY,
          operationId,
      ...
      if (isZKError(error)) {
        zkErrorLogger.logError(error, { 
          context: 'TrustedSetupManager.finalizeCeremony',
          ceremonyId,
          processingTimeMs: Date.now() - finalizationStartTime
        });
        throw error;
      ...
      // Otherwise, wrap in SystemError
      const systemError = new SystemError(`Ceremony finalization failed: ${error.message}`, {
        code: ErrorCode.SYSTEM_NOT_INITIALIZED,
        operationId,
        recoverable: false,
        details: {
  ...
  async distributeVerificationKey(ceremonyId, verificationKey, distributionConfig) {
    ...
    const operationId = `distribute_key_${distributionStartTime}`;
    ...
    try {
      ...
      for (const channel of distributionConfig.channels) {
        try {
          ...
          if (!channelMap) {
            throw new InputError(`Distribution channel ${channel} not found`, {
              code: ErrorCode.INPUT_VALIDATION_FAILED,
              operationId,
              recoverable: true,
              userFixable: true,
              details: {
                channel,
                availableChannels: Array.from(this.distributionChannels.keys())
              }
            });
          }
          ...
          if (!distributionConfig.redundancy) {
            ...
            throw new SystemError(`Failed to distribute to channel ${channel}: ${error.message}`, {
              code: ErrorCode.SYSTEM_NOT_INITIALIZED,
              operationId,
              recoverable: false,
              details: {
      ...
      if (successfulChannels.length === 0) {
        throw new SystemError('Failed to distribute verification key to any channel', {
          code: ErrorCode.SYSTEM_NOT_INITIALIZED,
          operationId,
          recoverable: false,
          details: {
      ...
      if (isZKError(error)) {
        zkErrorLogger.logError(error, { 
          context: 'TrustedSetupManager.distributeVerificationKey',
          ceremonyId,
          keyId: verificationKey.id,
          processingTimeMs: Date.now() - distributionStartTime
        });
      ...
      // Otherwise, wrap in SystemError
      const systemError = new SystemError(`Verification key distribution failed: ${error.message}`, {
        code: ErrorCode.SYSTEM_NOT_INITIALIZED,
        operationId,
        recoverable: false,
        details: {
  ...
  registerVerificationKey(ceremonyId, verificationKey) {
    ...
    try {
      ...
      this.auditLogger.log('Verification key registered successfully', {
        keyId,
        ceremonyId,
      ...
      // Log error with audit logger
      this.auditLogger.logError('Verification key registration failed', {
        ceremonyId,
        keyId: verificationKey.id || 'unknown',
        error: error.message
      });
      ...
      if (isZKError(error)) {
        zkErrorLogger.logError(error, { 
          context: 'TrustedSetupManager.registerVerificationKey',
          ceremonyId,
          keyId: verificationKey.id || 'unknown'
        });
        throw error;
      ...
      // Otherwise, wrap in SystemError
      const systemError = new SystemError(`Failed to register verification key: ${error.message}`, {
        code: ErrorCode.SYSTEM_NOT_INITIALIZED,
        operationId,
        recoverable: true,
        details: {
  ...
  getVerificationKey(keyId) {
    const operationId = `get_key_${Date.now()}`;
    
    try {
      // Get from registry
      const entry = this.verificationKeyRegistry.get(keyId);
      if (!entry) {
        throw new InputError(`Verification key ${keyId} not found`, {
          code: ErrorCode.INPUT_VALIDATION_FAILED,
          operationId,
          recoverable: false,
          userFixable: true,
      ...
      if (isZKError(error)) {
        zkErrorLogger.logError(error, { 
          context: 'TrustedSetupManager.getVerificationKey',
          keyId
        });
        throw error;
      }
      ...
      // Otherwise, wrap in InputError
      const inputError = new InputError(`Failed to get verification key: ${error.message}`, {
        code: ErrorCode.INPUT_VALIDATION_FAILED,
        operationId,
        recoverable: false,
        userFixable: true,
  ...
  getCeremonyStatus(ceremonyId) {
    const operationId = `get_ceremony_status_${Date.now()}`;
    ...
    try {
      ...
      if (isZKError(error)) {
        zkErrorLogger.logError(error, { 
          context: 'TrustedSetupManager.getCeremonyStatus',
          ceremonyId
        });
        throw error;
      }
      ...
      // Otherwise, wrap in InputError
      const inputError = new InputError(`Failed to get ceremony status: ${error.message}`, {
        code: ErrorCode.INPUT_VALIDATION_FAILED,
        operationId,
        recoverable: false,
        userFixable: true,
  ...
  listCeremonies(filters = {}) {
    try {
      ...
      const operationId = `list_ceremonies_${Date.now()}`;
      
      // Log error with audit logger
      this.auditLogger.logError('Failed to list ceremonies', {
        filters,
        error: error.message
      });
      ...
      if (isZKError(error)) {
        zkErrorLogger.logError(error, { 
          context: 'TrustedSetupManager.listCeremonies',
          filters
        });
        throw error;
      }
      ...
      // Otherwise, wrap in SystemError
      const systemError = new SystemError(`Failed to list ceremonies: ${error.message}`, {
        code: ErrorCode.SYSTEM_NOT_INITIALIZED,
        operationId,
        recoverable: true,
        details: {
  ...
  async generateVerificationKey(parameters, ceremony) {
    ...
    const operationId = `generate_key_${generationStartTime}`;
    ...
    try {
      ...
      if (!parameters || typeof parameters !== 'object') {
        throw new InputError('Invalid parameters for verification key generation', {
          code: ErrorCode.INPUT_TYPE_ERROR,
          operationId,
          recoverable: false,
          userFixable: false,
      ...
      if (isZKError(error)) {
        zkErrorLogger.logError(error, { 
          context: 'TrustedSetupManager.generateVerificationKey',
          keyId,
          ceremonyId: ceremony?.id,
          generationTimeMs: Date.now() - generationStartTime
        });
      ...
      // Otherwise, wrap in SystemError
      const systemError = new SystemError(`Failed to generate verification key: ${error.message}`, {
        code: ErrorCode.SYSTEM_NOT_INITIALIZED,
        operationId,
        recoverable: false,
        details: {
  ...
  async backupCeremonyParameters(ceremony) {
    const operationId = `backup_params_${Date.now()}`;
    ...
    try {
      ...
      this.auditLogger.log('Created secure backups', {
        ceremonyId: ceremony.id,
        backupCount: backups.length
      });
    } catch (error) {
      // Log error with audit logger
      this.auditLogger.logError('Failed to create backups', {
        ceremonyId: ceremony.id,
        error: error.message
      });
      ...
      if (isZKError(error)) {
        zkErrorLogger.logError(error, { 
          context: 'TrustedSetupManager.backupCeremonyParameters',
          ceremonyId: ceremony.id
        });
        throw error;
      }
      
      // Otherwise, wrap in SecurityError
      const securityError = new SecurityError(`Failed to create secure backups: ${error.message}`, {
        code: ErrorCode.SECURITY_DATA_INTEGRITY,
        operationId,
        recoverable: true,
        details: {
