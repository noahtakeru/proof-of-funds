 ...
 * used to detect vulnerabilities in ZK proof implementations. The security
 * rule system is a key component of the ZK security framework, enabling
 * automated detection of common security issues in ZK circuits, protocols,
 * and implementations.
 * 
 * The security rule system is designed to:
 * 1. Detect circuit vulnerabilities like constraint weaknesses
 * 2. Identify cryptographic implementation issues
 * 3. Find protocol-level security flaws
 * 4. Detect common coding practices that could lead to vulnerabilities
 * 5. Ensure proper input validation for ZK proof generation
 * 
 * Each security rule focuses on a specific vulnerability pattern or best practice,
 * and the rule engine applies all applicable rules to analyze ZK code.
 * 
 ...
 * 
 * Used when a security rule encounters invalid input during the validation
 * process. These errors typically indicate issues with the target being
 * evaluated rather than with the rule itself.
 * 
 * @extends Error
 */
 ...
 * 
 * Used when a security rule encounters an error during execution.
 * These errors typically indicate issues with the rule implementation
 * rather than with the target being evaluated.
 * 
 * @extends Error
 */
 ...
 * 
 * Used when a security rule is improperly configured. These errors
 * indicate issues with the rule setup rather than with execution
 * or validation.
 * 
 * @extends Error
 */
 ...
 * 
 * Security rules work by analyzing code, configurations, or runtime behavior to
 * identify potential security issues. Each rule focuses on a specific vulnerability
 * pattern or best practice, and produces structured findings when issues are detected.
 * 
 * @example
 * // Creating a custom security rule
 * class InsecureRandomnessRule extends SecurityRule {
 *   constructor() {
 *     super({
 *       id: 'ZK-SEC-RAND-001',
 *       name: 'Insecure Randomness Source',
 *       description: 'Detects use of insecure randomness sources in ZK systems',
 *       severity: 'high'
 *     });
 *   }
 *   
...
export class SecurityRule {
     ...
     * 
     * This is the core method that derived classes must implement to perform
     * the actual security analysis. The method analyzes the target code or
     * function and returns an array of findings for any detected issues.
     * 
     ...
     * 
     * Helper method for creating standardized finding objects. A finding
     * represents a specific instance of a security issue detected by the rule.
     * Each finding includes information about the issue location, severity,
     ...
     * 
     * Utility method for converting a character position to line and column
     * numbers. This is used to provide precise location information in findings.
     * 
