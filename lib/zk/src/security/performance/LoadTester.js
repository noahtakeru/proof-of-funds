...
class LoadTester {
  ...
  _executeOperation(operationFn, context) {
    ...
    const timeoutPromise = new Promise((_, reject) => {
      setTimeout(() => reject(new Error(`Operation timed out after ${this.options.timeout}ms`)), this.options.timeout);
    });
  ...
  const test = new LoadTester({
    ...
    setup: async () => {
      if (!components.zkProofGenerator || !components.zkUtils) {
        throw new Error('zkProofGenerator and zkUtils components are required for this test');
      }
