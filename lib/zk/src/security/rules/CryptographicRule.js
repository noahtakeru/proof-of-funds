...
/**
 * @class CryptographicRule
 * @classdesc Security rule for validating cryptographic implementations in ZK proof systems
 * This rule identifies potential security vulnerabilities in cryptographic implementations 
 * including weak algorithms, inadequate key sizes, and potential side-channel vulnerabilities.
 * 
...
export class CryptographicRule extends SecurityRule {
    ...
    checkPrimitiveStrength(primitiveType, params) {
        try {
            ...
            zkErrorLogger.logError(error, {
                context: 'CryptographicRule.checkPrimitiveStrength',
                ruleId: this.id,
                primitiveType,
    ...
    checkSideChannelVulnerabilities(code) {
        try {
            ...
            // Log the error
            zkErrorLogger.logError(error, {
                context: 'CryptographicRule.checkSideChannelVulnerabilities',
                ruleId: this.id,
    ...
    evaluate(context) {
        try {
            ...
            zkErrorLogger.logError(error, {
                context: 'CryptographicRule.evaluate',
                ruleId: this.id,
                filePath: context?.filePath,
                message: 'Error evaluating cryptographic rule'
            });
