 ...
 * scans ZK codebase for potential vulnerabilities. The auditor uses a rule-based
 * approach to detect common security issues in ZK proof systems, including:
 * 
 ...
 * 
 * The SecurityAuditor forms the backbone of the security assurance framework
 * for the ZK infrastructure, providing automated detection of security issues
 ...
 * const results = await auditor.auditFile('path/to/file.js');
 * console.log(results.getSummary());
 * 
 * // Audit a directory with custom rules
 * const customAuditor = new SecurityAuditor({ 
 *   rules: myCustomRules,
 *   includeDefault: true
 * });
 * const results = await customAuditor.auditDirectory('src/zk');
 */
 ...
 * 
 * Findings are immutable objects that serve as the core data structure for
 * tracking and reporting security issues. They provide rich metadata for
 * prioritization, remediation tracking, and integration with security tools.
 * 
 * @exports
 */
export class Finding {
     ...
     * This creates a standardized location format that can be used in reports,
     * logs, and integrations with other tools (like IDE plugins or CI systems).
     * 
     ...
     * Provides a human-readable representation of the finding for logging
     * and console output.
     * 
     ...
     * Used for serialization, report generation, and API responses.
     * 
 ...
 * 
 * SecurityRule is an abstract base class that establishes the foundation for
 * all security detection logic. Each concrete implementation focuses on a 
 * specific vulnerability pattern or security best practice.
 * 
 * @exports
 */
export class SecurityRule {
     ...
     * the actual security analysis. It should examine the file content and
     * return any findings.
     * 
     * Performance note: For large files, implementations should use efficient
     * algorithms to minimize processing time.
     * 
     ...
     * Helper method to create Finding objects with consistent data based on
     * the rule's properties. This standardizes finding creation across the
     * security framework.
     * 
     * @param {Object} options - Finding details
 ...
 * 
 * AuditResult serves as a comprehensive container for all information gathered
 * during an audit. It provides methods for filtering, analyzing, and reporting
 * on the findings to support security remediation workflows.
 * 
 * @exports
 */
export class AuditResult {
     ...
     * Used to prioritize remediation efforts based on severity.
     * 
     ...
     * Used to group related findings for focused remediation.
     * 
     ...
     * Used to identify all issues in a specific file for targeted fixes.
     * 
