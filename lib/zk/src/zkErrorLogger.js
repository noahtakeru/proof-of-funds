/**
 * ZK Error Logging and Aggregation System
 * 
 * This module provides structured error logging capabilities with privacy preservation
 * and error aggregation for analysis and monitoring.
 * 
 * Key features:
...
// Try to dynamically import zkErrorHandler for ESM context
try {
  // Using dynamic import to support ESM
  const importModule = async () => {
    const module = await import('./zkErrorHandler.js');
    zkErrorHandler = module.default;
  };

  // Immediately execute the import
  importModule().catch(err => {
    console.error('Error importing zkErrorHandler in ESM mode:', err);
  });
} catch (err) {
  // Fallback to CommonJS require
  try {
    zkErrorHandler = require('./zkErrorHandler');
  } catch (requireErr) {
    console.error('Error importing zkErrorHandler in CommonJS mode:', requireErr);
    // Provide fallback values
    zkErrorHandler = {
      isZKError: (error) => error && error.name && error.name.startsWith('ZK'),
      fromError: (error, options = {}) => error,
      ErrorSeverity: {
        CRITICAL: 'critical',
        ERROR: 'error',
        WARNING: 'warning',
...
const isZKError = () => zkErrorHandler?.isZKError || 
  ((error) => error && error.name && error.name.startsWith('ZK'));
const fromError = () => zkErrorHandler?.fromError || 
  ((error, options = {}) => error);
const ErrorSeverity = zkErrorHandler?.ErrorSeverity || {
  CRITICAL: 'critical',
  ERROR: 'error',
  WARNING: 'warning',
  INFO: 'info'
...
// Privacy levels
const PrivacyLevel = {
  // Only log error code and generic message
  HIGH: 'high',
  // Log error details but sanitize potentially sensitive information
  MEDIUM: 'medium',
  // Log all error information including stack traces (development only)
  LOW: 'low'
};
...
// Create simplified telemetry API for error patterns
const errorTelemetry = {
  /**
   * Get error frequency by code
   * @returns {Array} Error frequencies by code
   */
  getErrorFrequency: () => {
    const patterns = zkErrorLogger.getErrorPatterns();
    return patterns.trendingCodes || [];
  },
  ...
  getCategoryDistribution: () => {
    const patterns = zkErrorLogger.getErrorPatterns();
    return patterns.categoryRanking || [];
  },
  ...
  getRecoveryMetrics: () => {
    const patterns = zkErrorLogger.getErrorPatterns();
    return {
      recoverablePercentage: patterns.recoverablePercentage || 0,
      userFixablePercentage: patterns.userFixablePercentage || 0
    };
  },
...
// Create the exports object with all exports
const exportObj = {
  zkErrorLogger,
  ZKErrorLogger,
  LogLevel,
...
// Dual-format export support
// Support both ESM and CommonJS export patterns
// Check if we have the exports object (CommonJS environment)
if (typeof exports !== 'undefined') {
  // CommonJS environment
  Object.assign(exports, exportObj);
