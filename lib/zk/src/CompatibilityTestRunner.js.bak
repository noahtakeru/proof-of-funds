...
class CompatibilityTestRunner {
  ...
  async runTests(categoryFilter) {
    const operationId = `runTests_${Date.now()}`;
    
    if (this.isRunning) {
      const error = new SystemError('Tests are already running', {
      ...
      zkErrorLogger.logError(error, { context: 'CompatibilityTestRunner.runTests' });
      throw error;
    }
    ...
    try {
      ...
      const testError = error instanceof SystemError ? error : new SystemError(
        `Error running compatibility tests: ${error.message}`,
        {
      ...
      // Log the error
      zkErrorLogger.logError(testError, { 
        context: 'CompatibilityTestRunner.runTests',
        browser: this.currentBrowser.name,
        platform: this.currentPlatform.name
      });
  ...
  async _runFeatureTest(category, feature) {
    const operationId = `runFeatureTest_${Date.now()}`;
    ...
    try {
      ...
      // Handle test errors with proper error class
      const testError = new SystemError(`Failed to run test for ${feature.name}: ${error.message}`, {
        code: ErrorCode.SYSTEM_EXECUTION_FAILED,
        operationId,
        recoverable: true,
        details: {
      ...
      // Log the error properly
      zkErrorLogger.logError(testError, { 
        context: 'CompatibilityTestRunner._runFeatureTest',
        feature: feature.name,
        browser: this.currentBrowser.name
      });
      
      // Update the result with error details
      result = {
        ...result,
