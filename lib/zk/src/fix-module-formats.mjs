/**
 * ESM Module Format Standardizer (ESM-specific implementation)
 * 
 ...
 */
export function convertToESM(filePath, renameFile = true) {
  ...
  try {
    ...
    // Replace named exports in CommonJS style
    content = content.replace(
      new RegExp('exports\\.(\\w+)\\s*=\\s*([^;]+);', 'g'),
      (match, exportName, exportValue) => {
        return `export const ${exportName} = ${exportValue};`;
      }
    );

    // Replace full CommonJS module exports
    content = content.replace(
      new RegExp('module\\.exports\\s*=\\s*([^;]+);', 'g'),
      (match, exportValue) => {
 ...
 */
export function buildCJSCompatibilityVersions() {
 ...
 */
export function updateImportPaths(directory = path.join(projectRoot, 'src')) {
  ...
  try {
    ...
    files.forEach(file => {
      try {
        ...
        content = content.replace(
          new RegExp('import\\s+(?:[\\w\\s{}*,]+\\s+from\\s+)?[\'"]([^\'"]+)\\.js[\'"]', 'g'),
          (match, importPath) => {
 ...
 */
export function standardizeModules() {
...
// Export needed classes for error handling
export { ModuleFormatError };