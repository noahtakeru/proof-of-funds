...
// Add specialized error codes if not already defined
...
/**
 * Specialized error class for session-related issues
 * Provides more context and standardization for session errors
 */
class SessionError extends SystemError {
  constructor(message, options = {}) {
    super(message, {
      code: options.code || ErrorCode.SESSION_GENERIC_ERROR,
      severity: options.severity || 'WARNING',
      recoverable: options.recoverable !== undefined ? options.recoverable : true,
      details: {
        ...(options.details || {}),
        component: 'SessionSecurity',
        operationId: options.operationId || `session_error_${Date.now()}`
      }
    });
...
class SessionSecurityManager {
  constructor(customSettings = {}) {
    ...
    // Start monitoring session expirations
   ...
   * @param {Object} additionalDetails - Additional details to include in error
   * @returns {Error} Specialized error with consistent formatting
   */
  handleSessionError(error, context, additionalDetails = {}) {
    // If it's already a specialized error, just augment it
    if (error instanceof AuthenticationError || error instanceof SessionError) {
      // Add any additional context information
      error.details = {
        ...error.details,
        ...additionalDetails,
        handledBy: 'SessionSecurityManager.handleSessionError'
      };

      // Log it
      zkErrorLogger.logError(error, { context });

      // Return it for throwing or handling
      return error;
    }

    // Create an appropriate specialized error based on message content
    let specializedError;

    if (error.message.includes('token') && error.message.includes('invalid')) {
      specializedError = new AuthenticationError(`Invalid token: ${error.message}`, {
      ...
      specializedError = new AuthenticationError(`Expired token: ${error.message}`, {
      ...
      specializedError = new SessionError(`Session expired: ${error.message}`, {
      ...
      // Generic fallback based on the type of operation
      const isAuth = context.includes('auth') || context.includes('token');

      if (isAuth) {
        specializedError = new AuthenticationError(`Authentication error: ${error.message}`, {
          code: ErrorCode.AUTH_GENERIC_ERROR,
          details: {
            originalError: error.message,
            ...additionalDetails
          },
        ...
        specializedError = new SessionError(`Session error: ${error.message}`, {
          code: ErrorCode.SESSION_GENERIC_ERROR,
          details: {
            originalError: error.message,
            ...additionalDetails
          },
    ...
    // Log the specialized error
    zkErrorLogger.logError(specializedError, { context });

    // Return for throwing or handling
    return specializedError;
  }
  ...
  startExpirationMonitoring() {
    ...
    zkErrorLogger.log('INFO', 'Session expiration monitoring started', {
      context: 'SessionSecurityManager.startExpirationMonitoring',
      details: { checkIntervalMs: 60000 }
    });
  }

  /**
   * Stop monitoring session expirations (for cleanup)
   */
  stopExpirationMonitoring() {
    if (this.expirationCheckInterval) {
      ...
      zkErrorLogger.log('INFO', 'Session expiration monitoring stopped', {
        context: 'SessionSecurityManager.stopExpirationMonitoring'
      });
    }
  }
   ...
   * @param {number|Date} expiresAt - Expiration time
   */
  setSessionExpiration(sessionId, expiresAt) {
    if (!sessionId) {
      throw new InputError('Session ID is required', {
        code: ErrorCode.INPUT_MISSING_REQUIRED,
    ...
    if (typeof expirationTime !== 'number' || expirationTime <= Date.now()) {
      throw new InputError('Session expiration must be in the future', {
        code: ErrorCode.INPUT_VALIDATION_FAILED,
        details: {
          parameter: 'expiresAt',
          received: expiresAt,
    ...
    this.sessionExpirations.set(sessionId, expirationTime);

    zkErrorLogger.log('INFO', 'Session expiration set', {
      context: 'SessionSecurityManager.setSessionExpiration',
      details: {
        sessionId,
   ...
   * @returns {Array<string>} List of expired session IDs that were processed
   */
  checkExpiredSessions() {
    ...
    this.sessionExpirations.forEach((expirationTime, sessionId) => {
    ...
    for (const sessionId of expiredSessions) {
      ...
      this.handleSessionExpiration(sessionId);

      zkErrorLogger.log('INFO', 'Session expired', {
        context: 'SessionSecurityManager.checkExpiredSessions',
        details: { sessionId, expirationTime: new Date(expirationTime) }
      });
    }

    // Return expired sessions for testing
  ...
  handleSessionExpiration(sessionId) {
    try {
      // Invalidate the session in storage if needed
      if (this.sessionStorage) {
        this.sessionStorage.invalidateSession(sessionId);
      }

      // If there's an event system, emit an event
      if (this.eventEmitter) {
        this.eventEmitter.emit('sessionExpired', { sessionId });
      }

      zkErrorLogger.log('INFO', 'Session expiration handled', {
        context: 'SessionSecurityManager.handleSessionExpiration',
        details: { sessionId }
      });
    } catch (error) {
      const handlingError = this.handleSessionError(
        error,
        'SessionSecurityManager.handleSessionExpiration',
        { sessionId }
      );
  ...
  /**
   * Validate a token with comprehensive error handling
   * @param {string} token - The token to validate
   * @param {Object} options - Options for validation
   * @param {string} [options.audience] - Expected audience for the token
   * @param {boolean} [options.allowExpired=false] - Whether to allow expired tokens
   * @returns {Promise<Object>} Token validation result
   */
  async validateToken(token, options = {}) {
    const operationId = `validate_token_${Date.now()}`;
    const { audience = null, allowExpired = false } = options;
    ...
    try {
      ...
      if (!token.includes('.') || token.split('.').length !== 3) {
        throw new AuthenticationError('Invalid token format', {
          ...
          details: {
            ...
            tokenFormat: token.includes('.') ? `parts=${token.split('.').length}` : 'no-dots'
          }
        });
      }
      ...
      let decodedToken;
      try {
        // Decode but don't verify yet
        // (This is just for extracting claims for error messages)
      ...
      // Verify token expiration before full verification
      // This gives a more specific error message
      if (!allowExpired && decodedToken.exp && decodedToken.exp * 1000 < Date.now()) {
        const expiredAt = new Date(decodedToken.exp * 1000);
        const currentTime = new Date();
        const expiredAgo = Math.round((currentTime - expiredAt) / 1000 / 60); // minutes

        throw new AuthenticationError('Token has expired', {
          operationId,
          code: ErrorCode.AUTH_EXPIRED_TOKEN,
          recoverable: true, // Can be fixed by getting a new token
          details: {
            expiredAt,
            currentTime,
            minutesAgo: expiredAgo,
      ...
      // Assume verifyToken is the core validation method
      const verificationResult = await this.verifyToken(token, { audience });

      // Verify audience if specified
      if (audience && decodedToken.aud !== audience) {
      ...
      // If it's already a specialized error, just log and rethrow
      if (error instanceof AuthenticationError) {
        zkErrorLogger.logError(error, {
          context: 'SessionSecurityManager.validateToken'
        });
        throw error;
      }

      // Otherwise, create a proper authentication error
      const authError = new AuthenticationError(`Token validation failed: ${error.message}`, {
        operationId,
        code: ErrorCode.AUTH_INVALID_TOKEN,
        details: {
          originalError: error.message,
      ...
      zkErrorLogger.logError(authError, {
        context: 'SessionSecurityManager.validateToken'
      });
  ...
  decodeToken(token) {
    try {
      // JWT tokens are base64url encoded data separated by dots
