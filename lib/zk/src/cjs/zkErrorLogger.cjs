...
const { ErrorSeverity } = zkErrorHandler;
  ...
  logToServer(errorData) {
    ...
    try {
      ...
      if (typeof window === 'undefined') {
        // Node.js environment - use a different approach for server logging
        // In a real implementation, this might use a server-side logging library
        console.log('[SERVER LOG]', serverData);
        return;
      }
      
      // Browser environment - use sendBeacon if available for reliable delivery even during page unload
...
// Also export the singleton directly as default
module.exports.default = { zkErrorLogger };