/**
 * Zero-Knowledge Circuit Definitions (ESM Version)
 * 
 ...
 * Maps proof types to their numeric values for use in circuit selection
...
// Circom code for Standard Proof circuit with real EdDSA verification
const standardCircuitCode = `
pragma circom 2.0.0;

// Import cryptographic primitives from circomlib
...
template StandardProof() {
    ...
    signal private input privateSignature[3]; // EdDSA signature components (R8x, R8y, S)
    signal private input privateOwnershipProof[2]; // Public key components (Ax, Ay)
    ...
    // Hash the private address using Poseidon
    component addressHasher = Poseidon(20);
    for (var i = 0; i < 20; i++) {
        addressHasher.inputs[i] <== privateAddress[i];
    }
    
    // Verify the address hash matches
    publicAddressHash === addressHasher.out;

    // Verify ownership using EdDSA signature
    // The message is the concatenation of address and amount
    component msgHasher = Poseidon(21);
    for (var i = 0; i < 20; i++) {
        msgHasher.inputs[i] <== privateAddress[i];
    }
    msgHasher.inputs[20] <== privateAmount;
    signal msgHash;
    ...
    signatureVerifier.Ax <== privateOwnershipProof[0];
    signatureVerifier.Ay <== privateOwnershipProof[1];
    signatureVerifier.R8x <== privateSignature[0];
    signatureVerifier.R8y <== privateSignature[1];
    signatureVerifier.S <== privateSignature[2];
    signatureVerifier.M <== msgHash;
    
    // If we reach this point, the signature was verified successfully
}

component main {public [publicAmount, publicAddressHash]} = StandardProof();
`;

// Circom code for Threshold Proof circuit with real EdDSA verification
...
template ThresholdProof() {
    ...
    greaterOrEqual.in[0] <== privateAmount;
...
// Circom code for Maximum Proof circuit with real EdDSA verification
...
template MaximumProof() {
    ...
    lessOrEqual.in[0] <== privateAmount;
    lessOrEqual.in[1] <== maximumAmount;
    lessOrEqual.out === 1;
 ...
 * @constant {Object} CIRCUIT_NAMES
 * @property {string} STANDARD - Name of the standard proof circuit
 * @property {string} THRESHOLD - Name of the threshold proof circuit
 * @property {string} MAXIMUM - Name of the maximum proof circuit
 */
