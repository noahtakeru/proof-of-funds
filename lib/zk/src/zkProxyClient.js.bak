...
// Use static imports via the dependency mapper to eliminate webpack warnings
import dependencyMapper from './utils/dependencyMapper';

// Import zkProofGenerator statically
import zkProofGenerator from './zkProofGenerator';

// Get telemetry module
const telemetryModule = dependencyMapper.getModule('./telemetry');
const { telemetry } = telemetryModule;

// Get browserCompatibility module
const browserCompatibilityModule = dependencyMapper.getModule('./browserCompatibility.mjs');
const { detectFeatures } = browserCompatibilityModule;

// Get snarkjsLoader module
import { snarkjsLoader } from './snarkjsLoader';

// Get error handling utilities
const errorHandlerModule = dependencyMapper.getModule('./zkErrorHandler.mjs');
const {
  ErrorCode,
  ErrorSeverity,
  NetworkError,
...
// Get error logger module
const zkErrorLoggerModule = dependencyMapper.getModule('./zkErrorLogger.mjs');
 ...
 * 
 * This version uses the static dependency mapper instead of dynamic imports,
 * which eliminates webpack warnings about critical dependencies.
 * 
 ...
 * @throws {DependencyError} If module is not found
 */
async function loadDependency(path, fallbackPath) {
  try {
    // Try to get the module from our static dependency mapper
    if (dependencyMapper.hasModule(path)) {
      return Promise.resolve(dependencyMapper.getModule(path));
    }
    
    // Log warning for unknown module - this helps identify modules that need to be added
    console.warn(`Module ${path} not found in dependency mapper, consider adding it`);
    
    // For modules not explicitly mapped, we need to throw an error
    throw new Error(`Module not statically mapped: ${path}`);
  } catch (importError) {
    ...
    if (fallbackPath && dependencyMapper.hasModule(fallbackPath)) {
      try {
        return Promise.resolve(dependencyMapper.getModule(fallbackPath));
      } catch (fallbackError) {
