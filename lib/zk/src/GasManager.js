...
export class GasManager {
  ...
  async getETHPrice() {
    ...
    try {
      ...
      // Use our direct CoinGecko implementation instead of relying on external modules
      const prices = await this.fetchPricesForSymbols(['ethereum'], 'usd');
      
      if (prices && prices.ethereum && prices.ethereum.usd) {
   ...
   * @param {string[]} symbols - Array of cryptocurrency IDs to fetch prices for
   * @param {string} currency - Currency to get prices in (e.g., 'usd')
   * @returns {Promise<Object>} Object with price data keyed by cryptocurrency ID
   */
  async fetchPricesForSymbols(symbols, currency = 'usd') {
    const operationId = `fetch_prices_${Date.now()}`;
    
    try {
      // Normalize symbols
      const normalizedSymbols = symbols.map(s => s.toLowerCase());
      const symbolsParam = normalizedSymbols.join('%2C');
      ...
      // Set up cache key
      const cacheKey = `price_${symbolsParam}_${currency}`;
      const cacheExpiry = 5 * 60 * 1000; // 5 minutes
      
      // Check for cached data
      const cachedData = this.getPriceCache(cacheKey);
      if (cachedData) {
        zkErrorLogger.log('INFO', 'Using cached price data', {
          operationId,
          component: 'GasManager',
          details: {
      ...
      if (!response.ok) {
        throw new NetworkError(`CoinGecko API returned status: ${response.status}`, {
          operationId,
      ...
      if (!data || typeof data !== 'object') {
        throw new GasManagerError('CoinGecko API returned invalid data format', {
          operationId,
          operation: 'fetchPricesForSymbols',
          code: ErrorCode.DATA_FORMAT_ERROR,
          severity: ErrorSeverity.WARNING,
      ...
      // Cache the response
      this.storePriceCache(cacheKey, data, cacheExpiry);
      
      // Count successful requests
      zkErrorLogger.log('INFO', 'Successfully fetched price data from CoinGecko', {
        operationId,
        component: 'GasManager',
        details: {
          symbols: normalizedSymbols,
      ...
      if (error.name === 'AbortError') {
        const timeoutError = new NetworkTimeoutError('CoinGecko API request timed out', {
          operationId,
          operation: 'fetchPricesForSymbols',
      ...
      symbols.forEach(symbol => {
        fallbackData[symbol.toLowerCase()] = { [currency]: this.getDefaultPriceForSymbol(symbol, currency) };
      });
  ...
  getPriceCache(key) {
    ...
    const cached = this._priceCache[key];
    if (!cached) return null;
    
    // Check if cache is expired
    if (Date.now() - cached.timestamp > cached.expiry) {
      delete this._priceCache[key];
      return null;
    }
   ...
   * @private
   */
  storePriceCache(key, data, expiry) {
    if (!this._priceCache) this._priceCache = {};
    
    this._priceCache[key] = {
      data,
      timestamp: Date.now(),
      expiry
    };
   ...
   */
  cleanupPriceCache() {
    if (!this._priceCache) return;
    
    const now = Date.now();
    Object.keys(this._priceCache).forEach(key => {
      const entry = this._priceCache[key];
  ...
  /**
   * Get default price for a cryptocurrency symbol when API fails
   * @param {string} symbol - Cryptocurrency symbol
   * @param {string} currency - Currency to get price in
   * @returns {number} Default price
   * @private
   */
  getDefaultPriceForSymbol(symbol, currency = 'usd') {
    // Default prices for common cryptocurrencies (April 2025 estimates)
    const defaults = {
      'ethereum': 2200,
      'bitcoin': 48000,
      'polygon': 0.65,
