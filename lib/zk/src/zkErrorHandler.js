 ...
 * Used when operations to serialize or deserialize ZK proofs fail.
 * 
 * @class
 * @extends ProofError
 */
export class ProofSerializationError extends ProofError {
   ...
   */
  constructor(message, options = {}) {
    super(message, { ...options, code: options.code || ErrorCode.PROOF_SERIALIZATION_ERROR });
    this.name = 'ProofSerializationError';
  }
}

/**
 * Check if an error is a ZKError or derived from ZKError
 * @param {Error} error - Error to check
 * @returns {boolean} True if error is a ZKError or derived from ZKError
 */
...
/**
 * Create a specific ZKError based on error code
 * @param {number} code - Error code
 * @param {string} message - Error message
 * @param {Object} options - Additional options
 * @returns {ZKError} Appropriate ZKError instance
 */
export function createZKError(code, message, options = {}) {
  const mergedOptions = { ...options, code };

  // Determine which error class to use based on code
  if (code >= 1000 && code < 2000) {
    return new CircuitError(message, mergedOptions);
  } else if (code >= 2000 && code < 3000) {
    return new ProofError(message, mergedOptions);
  } else if (code >= 3000 && code < 4000) {
    return new VerificationError(message, mergedOptions);
  } else if (code >= 4000 && code < 5000) {
    return new MemoryError(message, mergedOptions);
  } else if (code >= 5000 && code < 6000) {
    return new NetworkError(message, mergedOptions);
  } else if (code >= 6000 && code < 7000) {
    return new SecurityError(message, mergedOptions);
  } else if (code >= 7000 && code < 8000) {
    return new InputError(message, mergedOptions);
  } else if (code >= 8000 && code < 9000) {
    return new SystemError(message, mergedOptions);
  } else if (code >= 9000 && code < 10000) {
    return new CompatibilityError(message, mergedOptions);
  } else {
    return new ZKError(message, mergedOptions);
  }
}

/**
 * Error class for circuit-related failures.
 * Used when circuit compilation or execution fails.
 * 
 * @class
 * @extends ZKError
 */
 ...
 * Used when memory allocation or management fails.
 * 
 * @class
 * @extends ZKError
 */
 ...
 * Used when environment or platform compatibility issues arise.
 * 
 * @class
 * @extends ZKError
 */
...
// Additional export for network timeout error
export class NetworkTimeoutError extends NetworkError {
  constructor(message, options = {}) {
    super(message, { ...options, code: options.code || ErrorCode.NETWORK_TIMEOUT });
