...
async function checkModule(name, requiredMethods) {
  ...
  try {
    ...
    // Check for files with different extensions based on our standardization
    const possibleExtensions = ['.js', '.cjs', '.mjs'];
    let filePath;
    let fileExists = false;
    
    for (const ext of possibleExtensions) {
      const testPath = path.join(basePath, `${name}${ext}`);
    ...
    // Fall back to .js extension if no file found
    if (!fileExists) {
      filePath = path.join(basePath, `${name}.js`);
    }
    
    // Check file existence
    if (!fs.existsSync(filePath)) {
      console.log(`❌ ${name} not found with any supported extension`);
      return false;
    }
    
    // Extract the actual file extension for logging
    const ext = path.extname(filePath);
    const foundFile = path.basename(filePath);
