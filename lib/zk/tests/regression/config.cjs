/**
 * Regression Test Configuration (CommonJS version)
 * 
...
// Warning files that should be fixed
const WARNING_FILES = {
  // Files with module system issues
  MODULE_SYSTEM: [
    'src/error-logging.js',
    'src/integration/APIEndpointTest.js',
    'src/integration/CrossComponentTest.js',
    'src/integration/E2EIntegrationTest.js',
    'src/realZkUtils.mjs',
    'src/security/PenetrationTest.js',
    'src/security/performance/LoadTester.js',
    'src/security/zkErrorHandler.js',
    'src/security/zkErrorLogger.js',
    'src/utils/ModuleStandardizer.js'
  ],
...
const WARNING_PATTERNS = {
  MODULE_SYSTEM: {
    // ESM files shouldn't use require
    ESM_WITH_REQUIRE: {
      pattern: /require\s*\(/,
      filePattern: /\.mjs$/,
      message: 'ESM file contains CommonJS requires'
    },
    // ESM files shouldn't use module.exports
    ESM_WITH_EXPORTS: {
      pattern: /module\.exports/,
      filePattern: /\.(js|mjs)$/,
      message: 'ESM file contains CommonJS exports'
    }
  },
  ...
  ERROR_HANDLING: {
    ...
    GENERIC_ERROR: {
      pattern: /new Error\s*\(/,
      message: 'Generic errors should be replaced with specific error classes'
    },
    // Should use error logging
    MISSING_ERROR_LOGGING: {
      pattern: /try\s*{[^}]*}\s*catch\s*\(\s*([a-zA-Z0-9_]+)\s*\)\s*{(?![^}]*zkErrorLogger|[^}]*\.logError)/,
      message: 'Try/catch blocks should use the error logging system'
    }
  }
};
