...
class ParameterValidator {
  ...
  async validateParameters(parameters, options = {}) {
    ...
    const parameterHash = this.hashData(parameters);
    const validationId = `validation-${parameterHash}-${Date.now()}`;
  ...
  addTrustedSource(sourceId, sourceInfo) {
    if (!sourceId || !sourceInfo || !sourceInfo.publicKey) {
      throw new Error('Source ID, name, and public key are required');
    }
  ...
  addTrustAnchor(keyId, anchorInfo) {
    if (!keyId || !anchorInfo) {
      throw new Error('Key ID and anchor information are required');
    }
