...
jest.mock('../../TrustedSetupManager.js', () => {
  ...
  return {
    ...
    submitContribution: jest.fn(async (ceremonyId, contribution) => {
      ...
      const calculatedHash = '0x' + require('js-sha3').sha256(JSON.stringify(contribution.parameters));
    ...
    finalizeCeremony: jest.fn(async (ceremonyId) => {
      ...
      ceremony.verificationKeyHash = '0x' + require('js-sha3').sha256(JSON.stringify(verificationKey));
    ...
    hashParameters: jest.fn((parameters) => {
      return '0x' + require('js-sha3').sha256(JSON.stringify(parameters));
    }),
    
    hashVerificationKey: jest.fn((key) => {
      return '0x' + require('js-sha3').sha256(JSON.stringify(key));
    })
  };
});
