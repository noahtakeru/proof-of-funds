...
import { DeploymentStrategyType, DeploymentStrategy } from '../src/deployment/DeploymentStrategySelector';
  ...
  return {
    DeploymentStrategySelector: jest.fn().mockImplementation(() => ({
      ...
      switchStrategy: jest.fn().mockImplementation((type) => ({
        type,
        useWorkerThreads: type !== 'server-side',
        workerThreadCount: type === 'high-performance' ? 8 : 4,
        useWebAssembly: true,
        serverOffloadPercentage: type === 'server-side' ? 100 :
          type === 'hybrid' ? 50 : 0,
        localProcessingOptimization: type === 'high-performance' ? 'performance' : 'balanced'
      })),
...
describe('CrossPlatformDeployment', () => {
  ...
  test('should retrieve status with correct properties', async () => {
    ...
    expect(status.strategy.type).toBe('hybrid');
