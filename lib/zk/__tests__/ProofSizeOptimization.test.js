...
const zkProofSerializer = require('../src/zkProofSerializer.mjs');
const proofSizeOptimization = require('../src/proof');
...
describe('Proof Size Optimization Module', () => {
  ...
  describe('ProofCompressor', () => {
    ...
    test('should create and extract compressed package', () => {
      // Create a compressed package
      const package = proofSizeOptimization.createCompressedProofPackage(sampleProof);
      
      // Validate the package
      expect(package).toBeDefined();
      expect(typeof package).toBe('string');
      
      // Parse the package to check its structure
      const parsedPackage = JSON.parse(package);
      expect(parsedPackage.type).toBe('compressed-zk-proof');
      expect(parsedPackage.data).toBeDefined();
      expect(parsedPackage.metadata).toBeDefined();
      
      // Extract the proof from the package
      const extracted = proofSizeOptimization.extractFromCompressedPackage(package);
      
      // Verify extraction restores the original proof
      expect(extracted).toEqual(sampleProof);
    });
