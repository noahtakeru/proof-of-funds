/**
 * Session Security Manager for Zero-Knowledge Proof System (CommonJS Version)
 * 
...
class SessionSecurityManager {
  ...
  hashSettings(settings) {
    ...
    try {
      if (typeof btoa === 'function') {
        ...
        // Node.js environment
        return Buffer.from(JSON.stringify(settings)).toString('base64').substring(0, 20);
      }
  ...
  async updateSessionData() {
    const operationId = `update_session_${Date.now()}`;
    ...
    try {
      ...
      try {
        ...
        throw new SecurityError('Failed to protect updated session data', {
          code: ErrorCode.SECURITY_DATA_INTEGRITY,
          operationId,
          securityCritical: true,
          details: { originalError: protectionError.message }
      ...
      // Update in sessionStorage if available
      if (typeof window !== 'undefined' && window.sessionStorage) {
        window.sessionStorage.setItem(`session-${this.sessionId}`,
          JSON.stringify(protectedSession));
      }
      ...
      if (error instanceof ZKError) {
        ...
        zkErrorLogger.logError(new SystemError('Failed to update session data', {
          code: ErrorCode.SYSTEM_RESOURCE_UNAVAILABLE,
          operationId,
          recoverable: true,
          details: { 
  ...
  async rotateSessionKeys() {
    const operationId = `rotate_session_keys_${Date.now()}`;
    ...
    try {
      ...
      zkErrorLogger.log('INFO', 'Starting scheduled key rotation', {
        operationId,
        sessionId: this.sessionId,
        keyCount: keyIds.length
      });
      ...
      for (const keyId of keyIds) {
        try {
          ...
          failureCount++;
          // Log individual key rotation errors but continue with others
          zkErrorLogger.logError(new SecurityError(`Failed to rotate key: ${keyId}`, {
            code: ErrorCode.SECURITY_KEY_ERROR,
            operationId,
            recoverable: true,
            details: { 
      ...
      zkErrorLogger.log('INFO', 'Completed scheduled key rotation', {
        operationId,
        sessionId: this.sessionId,
        keyCount: keyIds.length,
        successCount,
      ...
      if (error instanceof ZKError) {
        ...
        zkErrorLogger.logError(new SystemError('Key rotation operation failed', {
          code: ErrorCode.SYSTEM_RESOURCE_UNAVAILABLE,
          operationId,
          recoverable: true,
          details: { 
  ...
  async rotateKey(keyId) {
    const operationId = `rotate_key_${Date.now()}_${keyId}`;
    
    try {
      const keyInfo = this.sessionKeys.get(keyId);
      if (!keyInfo) {
        throw new SecurityError(`Key not found for rotation: ${keyId}`, {
          code: ErrorCode.SECURITY_KEY_ERROR,
          operationId,
          recoverable: false,
          details: { keyId }
      ...
      let newPassword;
      try {
        newPassword = this.secureKeyManager.generateSecurePassword();
      } catch (passwordError) {
        throw new SecurityError('Failed to generate new password for key rotation', {
          code: ErrorCode.SECURITY_KEY_ERROR,
          operationId,
          securityCritical: true,
          details: { 
            keyId,
            originalError: passwordError.message 
          }
        });
      }
      ...
      try {
        switch (keyInfo.type) {
          ...
          case 'input':
            ...
            break;
          default:
            throw new InputError(`Unknown key type: ${keyInfo.type}`, {
        ...
        throw new SecurityError(`Failed to retrieve data for key rotation: ${keyId}`, {
          code: ErrorCode.SECURITY_DATA_INTEGRITY,
          operationId,
          securityCritical: true,
          details: { 
      ...
      if (!data) {
        throw new SecurityError('Retrieved null or empty data during key rotation', {
          code: ErrorCode.SECURITY_DATA_INTEGRITY,
          operationId,
          securityCritical: true,
          details: { 
      ...
      try {
        ...
        throw new SystemError(`Failed to store rotated data: ${keyId}`, {
          code: ErrorCode.SYSTEM_RESOURCE_UNAVAILABLE,
          operationId,
          recoverable: false,
          details: { 
      ...
      try {
        ...
        // Log but don't fail the rotation if old data removal fails
        zkErrorLogger.logError(new SystemError('Failed to remove old data during key rotation', {
          code: ErrorCode.SYSTEM_RESOURCE_UNAVAILABLE,
          operationId,
          recoverable: true,
          details: { 
            ...
            newKeyId,
            keyType: keyInfo.type,
            originalError: removalError.message
          }
        }));
      }
      ...
      zkErrorLogger.log('INFO', 'Key rotated successfully', {
        operationId,
        sessionId: this.sessionId,
        keyType: keyInfo.type,
        keyId: keyId,
      ...
      if (error instanceof ZKError) {
        ...
        // Create and log a new ZKError
        const zkError = new SecurityError(`Failed to rotate key: ${error.message}`, {
          code: ErrorCode.SECURITY_KEY_ERROR,
          operationId,
          securityCritical: true,
          details: { 
  ...
  getKey(keyId) {
    ...
    if (!this.sessionId) {
      const error = new SystemError('Cannot get key: No active session', {
        code: ErrorCode.SYSTEM_NOT_INITIALIZED,
        operationId,
        userFixable: true,
        recoverable: true,
        recommendedAction: 'Initialize a session before accessing keys'
      });
    ...
    // Input validation
    if (!keyId) {
      const error = new InputError('Key ID is required', {
        code: ErrorCode.INPUT_MISSING_REQUIRED,
        operationId,
        userFixable: true,
    ...
    const keyInfo = this.sessionKeys.get(keyId);
    if (!keyInfo) {
      const error = new SecurityError(`Key not found: ${keyId}`, {
        code: ErrorCode.SECURITY_KEY_ERROR,
        operationId,
        userFixable: false,
        recoverable: false,
    ...
    zkErrorLogger.log('INFO', 'Key accessed', {
      operationId,
      sessionId: this.sessionId,
      keyType: keyInfo.type,
      keyId: keyId
  ...
  async rotateSensitiveOperationKey(keyId, operation) {
    const operationId = `rotate_sensitive_key_${Date.now()}`;
    ...
    try {
      zkErrorLogger.log('INFO', 'Rotating key after sensitive operation', {
        operationId,
        sessionId: this.sessionId,
        keyId: keyId,
        operation: operation
      ...
      zkErrorLogger.log('INFO', 'Key rotated successfully after sensitive operation', {
        operationId,
        sessionId: this.sessionId,
        keyId: keyId,
        newKeyId: result.newKeyId,
      ...
      if (error instanceof ZKError) {
        ...
        zkErrorLogger.logError(new SecurityError('Failed to rotate key after sensitive operation', {
          code: ErrorCode.SECURITY_KEY_ERROR,
          operationId,
          securityCritical: false, // Not critical since we can continue with existing key
          recoverable: true,
          details: { 
            keyId,
            operation,
  ...
  terminateSession(reason = 'user_requested') {
    const operationId = `terminate_session_${Date.now()}`;
    ...
    try {
      ...
      zkErrorLogger.log('INFO', 'Session terminating', {
        operationId,
        sessionId: oldSessionId,
        reason: reason
      });
      ...
      for (const keyId of keyIds) {
        try {
          ...
          zkErrorLogger.logError(new SystemError(`Failed to unregister key during session termination: ${keyId}`, {
            code: ErrorCode.SYSTEM_RESOURCE_UNAVAILABLE,
            operationId,
            recoverable: true,
            details: {
              keyId,
              originalError: unregisterError.message
            }
          }));
        }
      }

      // Remove session data if in browser environment
      if (typeof window !== 'undefined' && window.sessionStorage) {
      ...
      zkErrorLogger.log('INFO', 'Session terminated successfully', {
        operationId,
        sessionId: oldSessionId,
        reason: reason
      });
      ...
      if (error instanceof ZKError) {
        ...
        zkErrorLogger.logError(new SystemError('Session termination error', {
          code: ErrorCode.SYSTEM_RESOURCE_UNAVAILABLE,
          operationId,
          recoverable: true,
          details: { 
