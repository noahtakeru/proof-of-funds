...
async function createErrorPropagationTest(options = {}) {
  ...
  try {
    ...
    // Rest of implementation similar to ESM version
    return {
      ...results,
      success: false,
...
/**
 * Simulates worker process termination during execution
 * @param {Function} fn - Function to execute
 * @param {Object} options - Options for termination simulation
 * @returns {Promise<any>} Function result or error
 * @throws {SystemError} When simulating worker termination
 */
async function withWorkerTerminationSimulation(fn, options = {}) {
  ...
  // If testing recovery, use checkpointing (simplified for CJS compatibility)
  if (config.recoveryTest && config.reason !== 'user_cancel') {
    // Simplified implementation that does basic checkpointing
    try {
      simulateTermination();
    } catch (error) {
      // Log the error but continue (simulating recovery)
      zkErrorLogger.logError(error, {
        context: 'withWorkerTerminationSimulation',
        recoverable: true
      });
      
      return fn(); // Execute function after "recovery"
    }
  } else {
...
// Export all test functions with full CommonJS compatibility
module.exports = {
