 ...
 * Forces MetaMask to show its account selection UI to ensure user consent
 ...
 * For EVM wallets, removes from tracking without actual disconnection
 ...
 */
export const generateStandardProofHash = (walletAddress, amount) => {
    try {
        // Match the contract's hashing logic using ethers.js
        const encodedData = ethers.utils.defaultAbiCoder.encode(
            ['address', 'uint256', 'uint8'],
            [walletAddress, amount, 0] // 0 for STANDARD proof type
        );
 ...
 * @returns {string} - The keccak256 hash as a hex string
 */
export const generateThresholdProofHash = (walletAddress, amount) => {
    try {
        // Match the contract's hashing logic using ethers.js
        const encodedData = ethers.utils.defaultAbiCoder.encode(
            ['address', 'uint256', 'uint8', 'string'],
            [walletAddress, amount, 1, 'threshold'] // 1 for THRESHOLD proof type
        );
        return ethers.utils.keccak256(encodedData);
    } catch (error) {
 ...
 * @returns {string} - The keccak256 hash as a hex string
 */
export const generateMaximumProofHash = (walletAddress, amount) => {
    try {
        // Match the contract's hashing logic using ethers.js
        const encodedData = ethers.utils.defaultAbiCoder.encode(
            ['address', 'uint256', 'uint8', 'string'],
            [walletAddress, amount, 2, 'maximum'] // 2 for MAXIMUM proof type
        );
        return ethers.utils.keccak256(encodedData);
    } catch (error) {
 ...
 * @returns {string} - The keccak256 hash as a hex string
 */
export const generateProofHash = (walletAddress, amount, proofType) => {
    switch (proofType) {
        case 0: // STANDARD
 ...
 * These functions provide the core wallet management and asset scanning functionality
 * for the Proof of Funds application across multiple blockchain networks
 */
export {
    formatAddress,
    connectMetaMask,
