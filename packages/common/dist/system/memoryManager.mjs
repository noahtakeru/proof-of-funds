...
import { ErrorCode, MemoryError, InsufficientMemoryError, SecurityError, SystemError, isZKError } from '../error-handling/zkErrorHandler.mjs';
    ...
    try {
        ...
        if (global?.gc && typeof global.gc === 'function') {
            try {
                ...
                zkErrorLogger.logError(new SystemError('Manual GC not available', {
 ...
 */
export const startMemoryMonitoring = (warningCallback, errorCallback, checkIntervalMs = 5000) => {
    ...
    try {
        ...
        const checkMemory = () => {
            try {
                ...
                zkErrorLogger.logError(new SystemError(`Error during memory check: ${checkError.message}`, {
                    code: ErrorCode.SYSTEM_RESOURCE_UNAVAILABLE,
                    operationId,
                    recoverable: true,
...
export const runWithMemoryControl = async (fn, requiredMemoryMB, options = {}) => {
    ...
    try {
        ...
        if (!memCheck.isAvailable && !options.forceRun) {
            const zkError = new InsufficientMemoryError(`Insufficient memory for operation: ${memCheck.details}`, {
                code: ErrorCode.MEMORY_INSUFFICIENT,
                operationId,
                recoverable: true,
                userFixable: true,
        ...
        try {
            ...
            zkErrorLogger.logError(new SystemError(`Failed to start memory monitoring: ${monitorError.message}`, {
                code: ErrorCode.SYSTEM_NOT_INITIALIZED,
                operationId,
                recoverable: true,
        ...
        try {
            ...
            try {
                ...
                zkErrorLogger.logError(new SystemError(`Error stopping memory monitoring: ${stopError.message}`, {
                    code: ErrorCode.SYSTEM_NOT_INITIALIZED,
                    operationId,
                    recoverable: true,
            ...
            try {
                ...
                // Just log this error, don't throw from finally
                zkErrorLogger.logError(new SystemError(`Error suggesting garbage collection: ${gcError.message}`, {
                    code: ErrorCode.SYSTEM_RESOURCE_UNAVAILABLE,
                    operationId,
                    recoverable: true,
            ...
            if (options.cleanup && typeof options.cleanup === 'function') {
                try {
                    ...
                    zkErrorLogger.logError(new SystemError(`Error in memory cleanup: ${cleanupError.message}`, {
                        code: ErrorCode.SYSTEM_RESOURCE_UNAVAILABLE,
                        operationId,
                        recoverable: true,
...
export { MEMORY_PRESSURE_THRESHOLD, LOW_MEMORY_THRESHOLD_MB, CRITICAL_MEMORY_THRESHOLD_MB, GC_INTERVAL_MS };
