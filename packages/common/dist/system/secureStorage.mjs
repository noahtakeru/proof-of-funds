...
import { SecurityError, InputError, SystemError, ErrorCode, getErrorLogger } from '../error-handling/zkErrorHandler.mjs';
...
class SecureStorage {
    ...
    async storeWallet(walletData, password, expiresIn = DEFAULT_EXPIRATION.TEMP_WALLET) {
        ...
        try {
            ...
            const encryptedPrivateKey = await this.secureKeyManager.encryptPrivateKey(walletData.privateKey, password);
            ...
            this.storage.setItem(`${STORAGE_PREFIXES.TEMP_WALLET}${walletId}`, JSON.stringify(storageObject));
    ...
    async getWallet(walletId, password) {
        ...
        try {
            ...
            // Decrypt private key
            const privateKey = await this.secureKeyManager.decryptPrivateKey(walletData.encryptedPrivateKey, password);
    ...
    async storeCircuitInput(inputData, password, expiresIn = DEFAULT_EXPIRATION.CIRCUIT_INPUT) {
        ...
        try {
            ...
            // Store in session storage
            this.storage.setItem(`${STORAGE_PREFIXES.CIRCUIT_INPUT}${inputId}`, JSON.stringify(storageObject));
    ...
    async getCircuitInput(inputId, password) {
        ...
        try {
            ...
            // Decrypt data
            const decryptedData = await this.secureKeyManager.decrypt(inputData.encryptedData, password);
    ...
    async storeSessionData(key, data, password, expiresIn = DEFAULT_EXPIRATION.SESSION_DATA) {
        ...
        try {
            ...
            // Store in session storage
            this.storage.setItem(`${STORAGE_PREFIXES.SESSION_DATA}${key}`, JSON.stringify(storageObject));
    ...
    async getSessionData(key, password) {
        ...
        try {
            ...
            // Decrypt data
            const decryptedData = await this.secureKeyManager.decrypt(sessionData.encryptedData, password);
