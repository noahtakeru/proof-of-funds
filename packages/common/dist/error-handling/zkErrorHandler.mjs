...
// Initialize as null and load later to avoid circular dependencies
let _zkErrorLogger = null;
// Function to explicitly initialize the logger (for browser environments)
export function initializeErrorLogger(logger) {
    if (!logger) {
        throw new Error('Cannot initialize error logger with null or undefined value');
    }
    _zkErrorLogger = logger;
}
// Get the error logger, initializing it if needed
function getErrorLogger() {
    // If the logger is already initialized, return it
    if (_zkErrorLogger) {
...
/**
 * Creates a new ZKError with the specified parameters
 * @param {number} code - The error code
 * @param {string} message - The error message
 * @param {Object} options - Additional options for the error
 * @returns {ZKError} A new ZKError instance
 */
const createZKError = (code, message, options = {}) => {
    try {
        // Map error code to category and create appropriate error type
        const errorOptions = { ...options, code };
        let errorInstance;
        // Create specific error types based on category
        const category = Math.floor(code / 1000);
        switch (category) {
            case 1: // Circuit errors
                if (code === ErrorCode.CIRCUIT_CONSTRAINT_FAILURE) {
                    errorInstance = new CircuitConstraintError(message, errorOptions);
                }
                else if (code === ErrorCode.CIRCUIT_COMPILATION_ERROR) {
                    errorInstance = new CircuitCompilationError(message, errorOptions);
                }
                else if (code === ErrorCode.CIRCUIT_PARAMETER_ERROR) {
                    errorInstance = new CircuitParameterError(message, errorOptions);
                }
                else if (code === ErrorCode.CIRCUIT_VERSION_MISMATCH) {
                    errorInstance = new CircuitVersionError(message, errorOptions);
                }
                else {
                    errorInstance = new CircuitError(message, errorOptions);
                }
                break;
            case 2: // Proof errors
                if (code === ErrorCode.PROOF_WITNESS_ERROR) {
                    errorInstance = new ProofWitnessError(message, errorOptions);
                }
                else if (code === ErrorCode.PROOF_SERIALIZATION_ERROR) {
                    errorInstance = new ProofSerializationError(message, errorOptions);
                }
                else if (code === ErrorCode.PROOF_INPUT_INVALID) {
                    errorInstance = new ProofInputError(message, errorOptions);
                }
                else {
                    errorInstance = new ProofError(message, errorOptions);
                }
                break;
            case 3: // Verification errors
                if (code === ErrorCode.VERIFICATION_KEY_MISSING) {
                    errorInstance = new VerificationKeyError(message, errorOptions);
                }
                else if (code === ErrorCode.VERIFICATION_PROOF_INVALID) {
                    errorInstance = new VerificationProofError(message, errorOptions);
                }
                else {
                    errorInstance = new VerificationError(message, errorOptions);
                }
                break;
            case 4: // Memory errors
                if (code === ErrorCode.MEMORY_INSUFFICIENT) {
                    errorInstance = new InsufficientMemoryError(message, errorOptions);
                }
                else {
                    errorInstance = new MemoryError(message, errorOptions);
                }
                break;
            case 5: // Network errors
                if (code === ErrorCode.NETWORK_TIMEOUT) {
                    errorInstance = new NetworkTimeoutError(message, errorOptions);
                }
                else if (code === ErrorCode.NETWORK_SERVER_ERROR) {
                    errorInstance = new NetworkServerError(message, errorOptions);
                }
                else if (code === ErrorCode.NETWORK_RATE_LIMIT) {
                    errorInstance = new RateLimitError(message, errorOptions);
                }
                else {
                    errorInstance = new NetworkError(message, errorOptions);
                }
                break;
            case 6: // Security errors
                errorInstance = new SecurityError(message, errorOptions);
                break;
            case 7: // Input errors
                errorInstance = new InputError(message, errorOptions);
                break;
            case 8: // System errors
                errorInstance = new SystemError(message, errorOptions);
                break;
            case 9: // Compatibility errors
                if (code === ErrorCode.COMPATIBILITY_WASM_UNAVAILABLE) {
                    errorInstance = new WebAssemblyError(message, errorOptions);
                }
                else {
                    errorInstance = new CompatibilityError(message, errorOptions);
                }
                break;
            default:
                errorInstance = new ZKError(message, errorOptions);
        }
        // Log the error creation for tracking
        getErrorLogger().logError(errorInstance, {
            context: 'zkErrorHandler.createZKError',
            errorCreationDetails: {
                code,
                type: errorInstance.constructor.name,
                category: Math.floor(code / 1000)
            }
        });
        return errorInstance;
    }
    catch (error) {
        // Handle errors in error creation
        getErrorLogger().logError(error, {
            context: 'zkErrorHandler.createZKError',
            metaError: true,
            errorCreationAttempt: {
                code,
                message,
                options: JSON.stringify(options)
            }
        });
        // Fallback to basic ZKError
        return new ZKError(`Error creating specialized error: ${message}`, {
...
/**
 * Standardized error factory for creating ZK errors
 * @type {Object}
 */
const ZKErrorFactory = {
    /**
     * Creates a circuit-related error
     * @param {string} message - Error message
     * @param {Object} options - Error options
     * @returns {CircuitError} Circuit error instance
     */
    createCircuitError: (message, options = {}) => {
        try {
            const error = new CircuitError(message, options);
            // Log error creation
            getErrorLogger().logError(error, {
                context: 'ZKErrorFactory.createCircuitError'
            });
            return error;
        }
        catch (err) {
            // Log error in error creation
            getErrorLogger().logError(err, {
                context: 'ZKErrorFactory.createCircuitError',
                metaError: true
            });
            // Return fallback error
            return new ZKError(`Failed to create circuit error: ${message}`, {
                code: ErrorCode.SYSTEM_NOT_INITIALIZED,
                details: { originalMessage: message, creationError: err.message }
            });
        }
    },
    /**
     * Creates a proof-related error
     * @param {string} message - Error message
     * @param {Object} options - Error options
     * @returns {ProofError} Proof error instance
     */
    createProofError: (message, options = {}) => {
        try {
            const error = new ProofError(message, options);
            // Log error creation
            getErrorLogger().logError(error, {
                context: 'ZKErrorFactory.createProofError'
            });
            return error;
        }
        catch (err) {
            ...
            // Return fallback error
            return new ZKError(`Failed to create proof error: ${message}`, {
                code: ErrorCode.SYSTEM_NOT_INITIALIZED,
                details: { originalMessage: message, creationError: err.message }
            });
        }
     ...
     * @param {string} message - Error message
     * @param {Object} options - Error options
     * @returns {VerificationError} Verification error instance
     */
    createVerificationError: (message, options = {}) => {
        try {
            const error = new VerificationError(message, options);
            // Log error creation
            getErrorLogger().logError(error, {
                context: 'ZKErrorFactory.createVerificationError'
            });
            return error;
        }
        catch (err) {
            ...
            // Return fallback error
            return new ZKError(`Failed to create verification error: ${message}`, {
                code: ErrorCode.SYSTEM_NOT_INITIALIZED,
                details: { originalMessage: message, creationError: err.message }
            });
        }
    ...
    /**
     * Creates a general ZK error from a code
     * @param {number} code - Error code
     * @param {string} message - Error message
     * @param {Object} options - Error options
     * @returns {ZKError} ZK error instance
     */
    createFromCode: (code, message, options = {}) => {
        return createZKError(code, message, options);
    },
    /**
     * Creates a ZK error from an existing error
     * @param {Error} error - Original error
     * @param {Object} options - Error options
     * @returns {ZKError} ZK error instance
     */
    createFromError: (error, options = {}) => {
        try {
            const zkError = fromError(error, options);
            // Log error conversion
            getErrorLogger().logError(zkError, {
                context: 'ZKErrorFactory.createFromError',
                originalError: isZKError(error) ?
                    'Already ZKError' :
            ...
            // Log error in error conversion
            getErrorLogger().logError(err, {
                context: 'ZKErrorFactory.createFromError',
                metaError: true
            });
            // Return fallback error
            return new ZKError(`Failed to convert error: ${error.message}`, {
                code: ErrorCode.SYSTEM_NOT_INITIALIZED,
                details: {
                    originalError: `${error.name}: ${error.message}`,
     ...
     */
    wrapWithErrorHandling: async (operation, options = {}) => {
        const context = options.context || 'ZKErrorFactory.wrapWithErrorHandling';
        try {
            return await operation();
        }
        catch (error) {
            // Convert to ZKError if needed
            const zkError = isZKError(error) ? error : fromError(error, options);
            // Log the error through the error logging system
            getErrorLogger().logError(zkError, {
                context,
                wrappedOperation: true,
                ...options.logContext
            });
            // Apply any additional error handling
            if (typeof options.onError === 'function') {
                try {
                    await options.onError(zkError);
                }
                catch (handlingError) {
                    // Log error in error handling
                    getErrorLogger().logError(handlingError, {
                        context: `${context}.onError`,
                        metaError: true,
                        originalError: zkError.message
                    });
                }
            }
            throw zkError;
...
// Export objects for ESM format
export { ErrorSeverity, ErrorCategory, ErrorCode, ErrorCode as ZKErrorCode, ZKError, CircuitError, CircuitConstraintError, CircuitCompilationError, CircuitParameterError, CircuitVersionError, ProofError, ProofWitnessError, ProofSerializationError, ProofInputError, VerificationError, VerificationKeyError, VerificationProofError, MemoryError, InsufficientMemoryError, NetworkError, NetworkTimeoutError, NetworkServerError, RateLimitError, SecurityError, InputError, SystemError, CompatibilityError, WebAssemblyError, isZKError, fromError, getLocalizedErrorMessage, createZKError, ZKErrorFactory, getErrorLogger // Export the getErrorLogger function
 };
...
export default {
    ...
    createZKError,
    ZKErrorFactory,
    getErrorLogger, // Add to default export
    initializeErrorLogger // This is already exported via export function
};
