...
import { executeWithRetry, createErrorResponse, queuedRequest, rateLimitStats, sanitizeTokenMetadata, generateTokenDisplayInfo, cache, createBatchProcessor, optimizeChainOrder, organizeAssetsByCrossChain, enhanceTokenWithChainData } from './apiHelpers.js';
// Import centralized chain mappings utility
import { getMoralisChainId, CHAIN_MORALIS_MAPPING, getChainName, CHAIN_NATIVE_TOKENS, CHAIN_IDS } from './chainMappings.js';
...
const getMoralisChain = (chain) => {
    ...
    if (typeof chain === 'string') {
        ...
        if (chainName.includes('arb')) {
            return 'arbitrum';
        }
        if (chainName.includes('opt')) {
            return 'optimism';
        }
    }
    // Default fallback
...
export const getWalletTokens = async (address, chain = 'ethereum', options = {}) => {
    // Default options
    const { includeZeroBalances = true, // Ensure we include zero balances by default for token-agnosticism
...
export const getTokenPricesWithMoralis = async (tokens, chain = '', options = {}) => {
    ...
    // Default options
    const { useCache = true, // Use price cache by default
     } = options;
    try {
        ...
        const tokensWithAddresses = uniqueTokens.filter(t => t.address && t.address !== '0xNative' && t.address !== '0xError');
            ...
            for (let i = 0; i < tokensWithAddresses.length; i += BATCH_SIZE) {
                ...
                await Promise.all(batch.map(async (token) => {
                    try {
                        ...
                        if (!token.address || token.address === '0xNative') {
                            return;
                        }
        ...
        const nativeTokens = uniqueTokens.filter(t => t.type === 'native' || t.address === '0xNative');
            ...
            for (let i = 0; i < nativeTokens.length; i += BATCH_SIZE) {
                ...
                await Promise.all(batch.map(async (token) => {
                    try {
                        ...
                        if (moralisChain === 'eth') {
                            wrappedAddress = wrappedTokenMap['eth'];
                        }
                        else if (moralisChain === 'polygon') {
                            wrappedAddress = wrappedTokenMap['matic'];
                        }
                        else if (moralisChain === 'bsc') {
                            wrappedAddress = wrappedTokenMap['bnb'];
                        }
                        else if (moralisChain === 'fantom') {
                            wrappedAddress = wrappedTokenMap['ftm'];
                        }
                        else if (moralisChain === 'avalanche') {
                            wrappedAddress = wrappedTokenMap['avax'];
                        }
                        if (wrappedAddress) {
...
export const getWalletAssetsWithValue = async (address, chain = 'ethereum', options = {}) => {
    // Default options
    const { includeZeroBalances = true, includePotentialSpam = true } = options;
    try {
        // Normalize chain input for consistent handling
        let chainName = chain;
        // Map chain ID to name consistently
        ...
        try {
            ...
            isTestnetEnvironment = (chainName === 'polygon-amoy' ||
                chainName === 'mumbai' ||
                chainName === 'goerli' ||
                chainName === 'sepolia');
        }
