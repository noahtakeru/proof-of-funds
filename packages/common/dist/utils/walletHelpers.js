    ...
    try {
        ...
        if (parsedData.wallets.metamask && Array.isArray(parsedData.wallets.metamask)) {
            ...
            if (parsedData.wallets.metamask.length > 0) {
                console.log('MetaMask wallet addresses:', parsedData.wallets.metamask.map(w => w.address || w.fullAddress || 'unknown'));
            }
        ...
        if (parsedData.wallets.phantom && Array.isArray(parsedData.wallets.phantom)) {
            ...
            if (parsedData.wallets.phantom.length > 0) {
                console.log('Phantom wallet addresses:', parsedData.wallets.phantom.map(w => w.address || w.fullAddress || 'unknown'));
            }
...
export async function saveWalletConnection(walletType, accounts) {
    ...
    try {
        ...
        if (walletType === 'metamask' || walletType === 'evm') {
            ...
            const existingAddresses = new Set(walletData.wallets.metamask.map(w => w.address?.toLowerCase() || w.fullAddress?.toLowerCase()));
            // Add each wallet
            for (const wallet of walletArray) {
                if (typeof wallet === 'string') {
                    ...
                    if (existingAddresses.has(addressLower)) {
                        ...
                        const existingWallet = walletData.wallets.metamask.find(w => (w.address?.toLowerCase() || w.fullAddress?.toLowerCase()) === addressLower);
                        if (existingWallet) {
                    ...
                    if (walletAddress && existingAddresses.has(walletAddress)) {
                        // Update connection time for existing wallet
                        const existingWallet = walletData.wallets.metamask.find(w => (w.address?.toLowerCase() || w.fullAddress?.toLowerCase()) === walletAddress);
                        if (existingWallet) {
                            existingWallet.connected = true;
                            existingWallet.connectedAt = new Date().toISOString();
                        }
            ...
            const walletArray = Array.isArray(accounts) ? accounts : [accounts];
            // Get existing addresses for deduplication
            const existingAddresses = new Set(walletData.wallets.phantom.map(w => w.address?.toLowerCase() || w.fullAddress?.toLowerCase()));
            // Add each wallet
            for (const wallet of walletArray) {
                if (typeof wallet === 'string') {
                    // If it's just an address string
                    ...
                    if (existingAddresses.has(addressLower)) {
                        // Update connection time for existing wallet
                        const existingWallet = walletData.wallets.phantom.find(w => (w.address?.toLowerCase() || w.fullAddress?.toLowerCase()) === addressLower);
                        if (existingWallet) {
                            existingWallet.connected = true;
                            existingWallet.connectedAt = new Date().toISOString();
                        }
                    ...
                    if (walletAddress && existingAddresses.has(walletAddress)) {
                        // Update connection time for existing wallet
                        const existingWallet = walletData.wallets.phantom.find(w => (w.address?.toLowerCase() || w.fullAddress?.toLowerCase()) === walletAddress);
                        if (existingWallet) {
                            existingWallet.connected = true;
                            existingWallet.connectedAt = new Date().toISOString();
                        }
...
export async function scanMultiChainAssets(wallets, options = {}) {
    ...
    // Default options
    const { chains: specificChains = null, includeZeroBalances = true, includePotentialSpam = true } = options;
    try {
        ...
        for (const chainName of chainsToScan) {
            ...
            try {
                ...
                for (const wallet of wallets) {
                    ...
                    const walletAssets = await moralisApi.getWalletAssetsWithValue(walletAddress, chainName, {
                        includeZeroBalances,
                        includePotentialSpam
                    });
        ...
        if (isTestEnv) {
            ...
            return import('./moralisApi.js').then(moralisApi => {
                return moralisApi.getWalletAssetsWithValue(wallets[0]?.address || wallets[0]?.fullAddress || '0xDemoAddress', 'ethereum', { includeZeroBalances, includePotentialSpam });
            }).catch(importError => {
...
export async function convertAssetsToUSD(assets) {
    ...
    try {
        ...
        const prices = await moralisApi.getTokenPricesWithMoralis(totalAssets.map(asset => ({ symbol: asset.symbol, chain: asset.chain })));
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (data && data.coins && data.coins.length > 0) {
                ...
                const exactMatch = data.coins.find(coin => coin.symbol.toLowerCase() === symbol.toLowerCase());
                if (exactMatch) {
