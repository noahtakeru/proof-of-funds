    ...
    while (attempt < maxRetries) {
        try {
            ...
            if (isRateLimited(error)) {
                rateLimitStats.recordRateLimit(error.apiName || 'unknown', error.response?.headers?.['x-rate-limit-remaining'] || 0);
            }
...
export function sanitizeTokenMetadata(token, chain) {
    ...
    if (token.logo) {
        sanitized.logo = token.logo;
    }
    if (token.possible_spam) {
        sanitized.possible_spam = !!token.possible_spam;
    }
    if (token.verified_contract) {
        sanitized.verified_contract = !!token.verified_contract;
    }
    if (token.error) {
        sanitized.error = token.error;
    }
    return sanitized;
}
/**
...
export function organizeAssetsByCrossChain(assets) {
    ...
    const crossChainAssets = Object.entries(groupedBySymbol).map(([symbol, tokens]) => {
        ...
        return {
            ...
            primaryInstance: tokens.reduce((primary, token) => (!primary || (token.usdValue > primary.usdValue)) ? token : primary, null)
        };
    });
