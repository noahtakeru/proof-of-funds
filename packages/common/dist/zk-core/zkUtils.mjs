...
import { ZKErrorCode, createZKError, ErrorSeverity, InputError, ProofError, VerificationError, ProofSerializationError, SystemError, NetworkError, SecurityError, isZKError } from '../error-handling/zkErrorHandler.mjs';
...
const adaptiveComputation = new AdaptiveComputation(resourceMonitor, resourceAllocator, {
    enabledStrategies: [
...
export const toFieldElement = async (value) => {
    try {
        ...
        if (typeof value === 'bigint') {
            ...
            throw createZKError(ZKErrorCode.INVALID_INPUT_TYPE, `Invalid value type for field element conversion: ${typeof value}`, {
...
export const padArray = (arr, length, padValue = 0) => {
    try {
        ...
        if (!Array.isArray(arr)) {
            throw createZKError(ZKErrorCode.INVALID_INPUT_TYPE, 'padArray requires an array as input', {
                severity: ErrorSeverity.ERROR,
...
export const serializeZKProof = (proof, publicSignals) => {
    try {
        ...
        if (!proof || typeof proof !== 'object') {
            throw createZKError(ZKErrorCode.INVALID_PROOF_FORMAT, 'Invalid proof object provided for serialization', {
                severity: ErrorSeverity.ERROR,
        ...
        if (!Array.isArray(publicSignals)) {
            throw createZKError(ZKErrorCode.INVALID_PUBLIC_SIGNALS, 'Public signals must be an array', {
                severity: ErrorSeverity.ERROR,
...
export const deserializeZKProof = (serializedProof, serializedPublicSignals) => {
    try {
        ...
        if (!serializedProof || typeof serializedProof !== 'object') {
            throw createZKError(ZKErrorCode.INVALID_PROOF_FORMAT, 'Invalid serialized proof provided', {
                severity: ErrorSeverity.ERROR,
        ...
        if (!Array.isArray(serializedPublicSignals)) {
            throw createZKError(ZKErrorCode.INVALID_PUBLIC_SIGNALS, 'Serialized public signals must be an array', {
                severity: ErrorSeverity.ERROR,
        ...
        if (error.name === 'SyntaxError' && error.message.includes('BigInt')) {
            ...
            const zkError = createZKError(ZKErrorCode.BIGINT_CONVERSION_ERROR, 'Failed to convert string to BigInt during proof deserialization', {
                severity: ErrorSeverity.ERROR,
...
export const generateZKProofHash = (proof, publicSignals) => {
    ...
    try {
        ...
        if (!proof || typeof proof !== 'object') {
            throw createZKError(ZKErrorCode.INVALID_PROOF_FORMAT, 'Cannot generate hash: Invalid proof object', {
                severity: ErrorSeverity.ERROR,
                recoverable: false,
                userFixable: true,
                details: {
        ...
        if (!Array.isArray(publicSignals)) {
            throw createZKError(ZKErrorCode.INVALID_PUBLIC_SIGNALS, 'Cannot generate hash: Public signals must be an array', {
                severity: ErrorSeverity.ERROR,
                recoverable: false,
                userFixable: true,
                details: {
        ...
        try {
            ...
            const serializationError = createZKError(ZKErrorCode.PROOF_SERIALIZATION_FAILED, `Failed to serialize proof for hashing: ${error.message}`, {
                severity: ErrorSeverity.ERROR,
                recoverable: false,
                userFixable: false,
                details: {
        ...
        try {
            ...
            const hashingError = createZKError(ZKErrorCode.PROOF_HASH_GENERATION_FAILED, `SHA3-256 hashing operation failed: ${error.message}`, {
                severity: ErrorSeverity.ERROR,
                recoverable: false,
                userFixable: false,
                details: {
        ...
        const genericError = createZKError(ZKErrorCode.PROOF_HASH_GENERATION_FAILED, `Failed to generate proof hash: ${error.message}`, {
            severity: ErrorSeverity.ERROR,
            recoverable: false,
            userFixable: false,
            details: {
...
export const stringifyBigInts = (obj) => {
    ...
    try {
        ...
        if (Array.isArray(obj)) {
            return obj.map((item, index) => {
                try {
                    ...
                    const conversionError = createZKError(ZKErrorCode.BIGINT_CONVERSION_ERROR, `Failed to stringify BigInt in array at index ${index}: ${error.message}`, {
        ...
        if (typeof obj === 'object') {
            ...
            for (const key in obj) {
                try {
                    ...
                    const propertyError = createZKError(ZKErrorCode.BIGINT_CONVERSION_ERROR, `Failed to stringify BigInt in object property '${key}': ${error.message}`, {
                        severity: ErrorSeverity.WARNING,
        ...
        const inputError = createZKError(ZKErrorCode.BIGINT_CONVERSION_ERROR, `Failed to stringify BigInt values: ${error.message}`, {
            severity: ErrorSeverity.ERROR,
            recoverable: false,
            userFixable: false,
            details: {
    ...
    try {
        ...
        if (typeof obj === 'string' && /^[0-9]+$/.test(obj)) {
            try {
                ...
                const conversionError = createZKError(ZKErrorCode.BIGINT_CONVERSION_ERROR, `Failed to convert string to BigInt: ${error.message}`, {
                    severity: ErrorSeverity.WARNING,
        ...
        if (Array.isArray(obj)) {
            return obj.map((item, index) => {
                try {
                    ...
                    // Create specific error for array item conversion failures
                    const conversionError = createZKError(ZKErrorCode.BIGINT_CONVERSION_ERROR, `Failed to parse BigInt in array at index ${index}: ${error.message}`, {
                        severity: ErrorSeverity.WARNING,
                        recoverable: true, // We'll continue with other items
                        userFixable: false,
                        details: {
        ...
        if (typeof obj === 'object') {
            ...
            for (const key in obj) {
                try {
                    ...
                    failedKeys.push(key);
                    // Create specific error for object property conversion failures
                    const propertyError = createZKError(ZKErrorCode.BIGINT_CONVERSION_ERROR, `Failed to parse BigInt in object property '${key}': ${error.message}`, {
                        severity: ErrorSeverity.WARNING,
                        recoverable: true, // We'll continue with other properties
                        userFixable: false,
                        details: {
        ...
        // Otherwise wrap in an appropriate error with rich context
        const inputError = createZKError(ZKErrorCode.BIGINT_CONVERSION_ERROR, `Failed to parse BigInt values: ${error.message}`, {
            severity: ErrorSeverity.ERROR,
            recoverable: false,
            userFixable: false,
            details: {
...
export const formatNumber = (value, options = {}) => {
    ...
    const { precision = 2, currency = false, compact = false } = options;
    let numValue;
    try {
        // Convert to number
        ...
        if (isNaN(numValue)) {
            throw createZKError(ZKErrorCode.INPUT_CONVERSION_FAILED, `Value cannot be converted to a number: ${value}`, {
                severity: ErrorSeverity.WARNING,
        ...
        const conversionError = createZKError(ZKErrorCode.INPUT_CONVERSION_FAILED, `Failed to convert value to number: ${error.message}`, {
            severity: ErrorSeverity.WARNING,
    ...
    try {
        ...
        const formattingError = createZKError(ZKErrorCode.SYSTEM_FEATURE_UNSUPPORTED, `Number formatting failed: ${error.message}`, {
            severity: ErrorSeverity.WARNING,
            recoverable: true, // We provide a fallback
...
export const generateZKProof = async (inputs, incomingProofType, options = {}) => {
    ...
    try {
        ...
        const computationResult = await adaptiveComputation.executeComputation(operationId, async (resources) => {
            // Validate inputs to prevent GIGO
            if (!inputs || typeof inputs !== 'object') {
                throw createZKError(ZKErrorCode.INVALID_INPUT, 'Invalid inputs: inputs must be an object', {
                    severity: ErrorSeverity.ERROR,
            ...
            if (!proofType) {
                throw createZKError(ZKErrorCode.INVALID_PROOF_TYPE, 'Empty proof type', {
                    severity: ErrorSeverity.ERROR,
                    details: {
            ...
            try {
                if (!isServer) {
                    ...
                    try {
                        ...
                        throw createZKError(ZKErrorCode.BROWSER_PROVING_ERROR, `Browser proof generation failed: ${error.message}`, {
                            severity: ErrorSeverity.ERROR,
                    ...
                    try {
                        ...
                        const { witness, wtnsFile } = await snarkjs.wtns.calculate(inputs, wasmFile, 
        ...
        if (!computationResult.success) {
            throw new ProofError(`Adaptive proof generation failed: ${computationResult.error?.message || 'Unknown error'}`, {
                code: ZKErrorCode.PROOF_GENERATION_FAILED,
                operationId,
                recoverable: false,
                details: {
...
async function verifyZKProofInternal(proof, publicSignals, proofType, operationId, options = {}) {
    try {
        // Validate inputs
        if (!proof || typeof proof !== 'object') {
            throw createZKError(ZKErrorCode.INVALID_PROOF_FORMAT, 'Invalid proof: proof must be an object with pi_a, pi_b, pi_c components', {
                severity: ErrorSeverity.ERROR,
                details: { proofType: typeof proof },
                recoverable: false,
                userFixable: true
        ...
        if (!Array.isArray(publicSignals)) {
            throw createZKError(ZKErrorCode.INVALID_PUBLIC_SIGNALS, 'Invalid public signals: must be an array', {
                severity: ErrorSeverity.ERROR,
        ...
        try {
            ...
            const verificationError = new VerificationError(`Proof verification failed: ${error.message}`, {
    ...
    try {
        ...
        if (isServer) {
            ...
            if (!safeFileOps.fileExists(filePath)) {
                throw createZKError(ZKErrorCode.VERIFICATION_KEY_NOT_FOUND, `Could not load verification key from ${vkeyPath}`, {
                    severity: ErrorSeverity.ERROR,
            ...
            try {
                ...
                throw createZKError(ZKErrorCode.VERIFICATION_KEY_NOT_FOUND, `Failed to fetch verification key: ${fetchError.message}`, {
                    severity: ErrorSeverity.ERROR,
