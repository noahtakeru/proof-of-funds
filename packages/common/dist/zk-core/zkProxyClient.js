...
export class RateLimiter {
    ...
    checkRateLimit(userId) {
        ...
        const allowed = minuteLimit.current < minuteLimit.max &&
    ...
    cleanup() {
        ...
        for (const userId in this.userRequests) {
            const user = this.userRequests[userId];
            const lastRequestTime = Math.max(...user.hourRequests.concat(user.minuteRequests, 0));
    ...
    getStatus() {
        return {
            ...
            totalRequests: Object.values(this.userRequests).reduce((total, user) => total + user.hourRequests.length, 0)
        };
    }
    /**
