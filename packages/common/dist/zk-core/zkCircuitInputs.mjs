...
import { ZKErrorCode, createZKError, ErrorSeverity } from '../error-handling/zkErrorHandler.mjs';
/**
...
export const addressToBytes = (address) => {
    try {
        ...
        if (!address || typeof address !== 'string') {
            throw createZKError(ZKErrorCode.INVALID_ADDRESS, 'Invalid Ethereum address: address must be a non-empty string', {
        ...
        if (cleanAddress.length !== 40) {
            throw createZKError(ZKErrorCode.INVALID_ADDRESS, `Invalid Ethereum address length: expected 40 hex chars, got ${cleanAddress.length}`, {
                severity: ErrorSeverity.ERROR,
                details: {
        ...
        if (!/^[0-9a-fA-F]+$/.test(cleanAddress)) {
            throw createZKError(ZKErrorCode.INVALID_ADDRESS, 'Invalid Ethereum address: must contain only hex characters', {
                severity: ErrorSeverity.ERROR,
                details: {
                    hasNonHexChars: true
                },
        ...
        if (!error.code || !error.name || error.name !== 'ZKError') {
            error = createZKError(ZKErrorCode.ADDRESS_CONVERSION_FAILED, `Failed to convert address to bytes: ${error.message}`, {
                severity: ErrorSeverity.ERROR,
...
export const extractPublicInputs = (inputs, proofType) => {
    try {
        ...
        if (!inputs || typeof inputs !== 'object') {
            throw createZKError(ZKErrorCode.INVALID_CIRCUIT_INPUTS, 'Invalid circuit inputs: must be a non-empty object', {
                severity: ErrorSeverity.ERROR,
                details: {
        ...
        if (!proofType || typeof proofType !== 'string') {
            throw createZKError(ZKErrorCode.INVALID_PROOF_TYPE, `Invalid proof type: ${proofType}`, {
                severity: ErrorSeverity.ERROR,
                details: {
        ...
        switch (proofType) {
            case ZK_PROOF_TYPES.STANDARD:
                ...
                if (!inputs.publicAmount) {
                    throw createZKError(ZKErrorCode.MISSING_PUBLIC_INPUT, 'Missing required public input: publicAmount', {
                        severity: ErrorSeverity.ERROR,
                ...
                if (!inputs.publicAddressHash) {
                    throw createZKError(ZKErrorCode.MISSING_PUBLIC_INPUT, 'Missing required public input: publicAddressHash', {
                        severity: ErrorSeverity.ERROR,
            ...
            case ZK_PROOF_TYPES.THRESHOLD:
                ...
                if (!inputs.thresholdAmount) {
                    throw createZKError(ZKErrorCode.MISSING_PUBLIC_INPUT, 'Missing required public input: thresholdAmount', {
                        severity: ErrorSeverity.ERROR,
            ...
            case ZK_PROOF_TYPES.MAXIMUM:
                ...
                if (!inputs.maximumAmount) {
                    throw createZKError(ZKErrorCode.MISSING_PUBLIC_INPUT, 'Missing required public input: maximumAmount', {
                        severity: ErrorSeverity.ERROR,
            ...
            default:
                throw createZKError(ZKErrorCode.INVALID_PROOF_TYPE, `Invalid proof type: ${proofType}. Expected one of: ${Object.values(ZK_PROOF_TYPES).join(', ')}`, {
                    severity: ErrorSeverity.ERROR,
                    details: {
    ...
    try {
            ...
            default:
                ...
                if (!proofType || !Object.values(ZK_PROOF_TYPES).includes(proofType)) {
                    return {
                        valid: false,
                        errors: {
                            proofType: createUserFriendlyError(ZKErrorCode.INVALID_PROOF_TYPE, `Invalid proof type: ${proofType}. Must be one of: ${Object.values(ZK_PROOF_TYPES).join(', ')}`, {
                                severity: ErrorSeverity.ERROR,
                                operationId,
                                details: {
                                    providedType: proofType,
        ...
        if (validationResults.valid) {
            ...
            if (proofType === ZK_PROOF_TYPES.STANDARD &&
                ...
                validationResults.errors.balance = createUserFriendlyError(ZKErrorCode.INVALID_CIRCUIT_INPUT, 'Balance must be greater than or equal to the specified amount', {
                    severity: ErrorSeverity.ERROR,
                    operationId,
                    details: {
                        fieldName: 'balance',
            ...
            if (proofType === ZK_PROOF_TYPES.THRESHOLD &&
                ...
                validationResults.valid = false;
                validationResults.errors.balance = createUserFriendlyError(ZKErrorCode.INVALID_CIRCUIT_INPUT, 'Balance must be greater than or equal to the threshold value', {
                    severity: ErrorSeverity.ERROR,
                    operationId,
                    details: {
                        fieldName: 'balance',
        ...
        const validationError = error.name === 'ZKError' ? error : createZKError(ZKErrorCode.CIRCUIT_INPUT_VALIDATION_FAILED, `Unexpected error validating inputs: ${error.message}`, {
            severity: ErrorSeverity.ERROR,
            operationId,
            details: { originalError: error.message },
            recoverable: false,
...
export const sanitizeCircuitInput = (input, fieldType, options = {}, fieldName = 'field') => {
    ...
    const { allowEmpty = false, maxStringLength = 1024, minNumber = Number.MIN_SAFE_INTEGER, maxNumber = Number.MAX_SAFE_INTEGER, allowNegative = false, } = options;
    try {
        ...
        if (input === null || input === undefined) {
            ...
            throw createZKError(ZKErrorCode.INVALID_CIRCUIT_INPUT, `${fieldName} cannot be null or undefined`, {
                severity: ErrorSeverity.ERROR,
                operationId,
                details: {
                    fieldName,
        ...
        switch (fieldType) {
            case 'string':
                ...
                if (stringValue.length === 0 && !allowEmpty) {
                    throw createZKError(ZKErrorCode.INVALID_CIRCUIT_INPUT, `${fieldName} cannot be empty`, {
                        severity: ErrorSeverity.ERROR,
                        operationId,
                        details: {
                            fieldName,
                ...
                if (stringValue.length > maxStringLength) {
                    throw createZKError(ZKErrorCode.INVALID_CIRCUIT_INPUT, `${fieldName} exceeds maximum length of ${maxStringLength} characters`, {
                        severity: ErrorSeverity.ERROR,
                        operationId,
                        details: {
                            fieldName,
            ...
            case 'number':
                ...
                if (typeof input === 'string') {
                    ...
                    if (!/^-?\d*\.?\d*$/.test(cleanedInput)) {
                        throw createZKError(ZKErrorCode.INVALID_CIRCUIT_INPUT, `${fieldName} contains invalid characters for a number`, {
                            severity: ErrorSeverity.ERROR,
                            operationId,
                            details: {
                                fieldName,
                ...
                if (isNaN(numValue) || !isFinite(numValue)) {
                    throw createZKError(ZKErrorCode.INVALID_CIRCUIT_INPUT, `${fieldName} is not a valid number`, {
                        severity: ErrorSeverity.ERROR,
                        operationId,
                        details: {
                            fieldName,
                ...
                if (!allowNegative && numValue < 0) {
                    throw createZKError(ZKErrorCode.INVALID_CIRCUIT_INPUT, `${fieldName} cannot be negative`, {
                        severity: ErrorSeverity.ERROR,
                        operationId,
                        details: {
                            fieldName,
                ...
                if (numValue < minNumber) {
                    throw createZKError(ZKErrorCode.INVALID_CIRCUIT_INPUT, `${fieldName} is below the minimum value of ${minNumber}`, {
                        severity: ErrorSeverity.ERROR,
                        operationId,
                        details: {
                            fieldName,
                ...
                if (numValue > maxNumber) {
                    throw createZKError(ZKErrorCode.INVALID_CIRCUIT_INPUT, `${fieldName} exceeds the maximum value of ${maxNumber}`, {
                        severity: ErrorSeverity.ERROR,
                        operationId,
                        details: {
                            fieldName,
            ...
            case 'address':
                ...
                if (typeof input !== 'string') {
                    throw createZKError(ZKErrorCode.INVALID_CIRCUIT_INPUT, `${fieldName} must be a string`, {
                        severity: ErrorSeverity.ERROR,
                        operationId,
                        details: {
                            fieldName,
                ...
                // Validate address length (should be 40 characters without prefix)
                if (cleanAddress.length !== 40) {
                    throw createZKError(ZKErrorCode.INVALID_CIRCUIT_INPUT, `${fieldName} has invalid length: expected 40 hex chars, got ${cleanAddress.length}`, {
                        severity: ErrorSeverity.ERROR,
                        operationId,
                        details: {
                            fieldName,
                ...
                // Validate address is hex format
                if (!/^[0-9a-fA-F]+$/.test(cleanAddress)) {
                    throw createZKError(ZKErrorCode.INVALID_CIRCUIT_INPUT, `${fieldName} must contain only hexadecimal characters`, {
                        severity: ErrorSeverity.ERROR,
                        operationId,
                        details: {
                            fieldName,
            ...
            case 'bigint':
                ...
                try {
                    ...
                    throw createZKError(ZKErrorCode.INVALID_CIRCUIT_INPUT, `${fieldName} is not a valid big integer: ${error.message}`, {
                        severity: ErrorSeverity.ERROR,
                        operationId,
                        details: {
                            fieldName,
                ...
                if (typeof options.minBigInt === 'bigint' && bigIntValue < options.minBigInt) {
                    throw createZKError(ZKErrorCode.INVALID_CIRCUIT_INPUT, `${fieldName} is below the minimum value`, {
                        severity: ErrorSeverity.ERROR,
                        operationId,
                        details: {
                            fieldName,
                ...
                if (typeof options.maxBigInt === 'bigint' && bigIntValue > options.maxBigInt) {
                    throw createZKError(ZKErrorCode.INVALID_CIRCUIT_INPUT, `${fieldName} exceeds the maximum value`, {
                        severity: ErrorSeverity.ERROR,
                        operationId,
                        details: {
                            fieldName,
            ...
            case 'array':
                ...
                if (!Array.isArray(input)) {
                    throw createZKError(ZKErrorCode.INVALID_CIRCUIT_INPUT, `${fieldName} must be an array`, {
                        severity: ErrorSeverity.ERROR,
                        operationId,
                        details: {
                            fieldName,
                ...
                if (options.minLength !== undefined && input.length < options.minLength) {
                    throw createZKError(ZKErrorCode.INVALID_CIRCUIT_INPUT, `${fieldName} must have at least ${options.minLength} items`, {
                        severity: ErrorSeverity.ERROR,
                        operationId,
                        details: {
                            fieldName,
                ...
                if (options.maxLength !== undefined && input.length > options.maxLength) {
                    throw createZKError(ZKErrorCode.INVALID_CIRCUIT_INPUT, `${fieldName} cannot have more than ${options.maxLength} items`, {
                        severity: ErrorSeverity.ERROR,
                        operationId,
                        details: {
                            fieldName,
                ...
                if (options.itemType) {
                    return input.map((item, index) => sanitizeCircuitInput(item, options.itemType, options.itemOptions || {}, `${fieldName}[${index}]`));
                }
                return input;
            default:
        ...
        const zkError = createZKError(ZKErrorCode.CIRCUIT_INPUT_SANITIZATION_FAILED, `Failed to sanitize ${fieldName}: ${error.message}`, {
            severity: ErrorSeverity.ERROR,
            operationId,
            details: {
                fieldName,
...
export const validateCircuitInput = (inputName, inputValue, constraints) => {
    ...
    try {
        ...
        if (constraints.required && (inputValue === undefined || inputValue === null)) {
            return {
                valid: false,
                error: createUserFriendlyError(ZKErrorCode.MISSING_PUBLIC_INPUT, `Missing required input: ${inputName}`, {
                    severity: ErrorSeverity.ERROR,
                    operationId,
        ...
        try {
            const sanitizedValue = sanitizeCircuitInput(inputValue, constraints.type, constraints.options || {}, inputName);
            ...
            if (constraints.type === 'number' || constraints.type === 'bigint') {
                ...
                if (inputName.toLowerCase().includes('amount') ||
                    ...
                    if (constraints.type === 'number' && sanitizedValue <= 0) {
                        return {
                            valid: false,
                            error: createUserFriendlyError(ZKErrorCode.INVALID_CIRCUIT_INPUT, `${inputName} must be greater than zero`, {
                                severity: ErrorSeverity.ERROR,
                                operationId,
                                details: {
                                    fieldName: inputName,
            ...
            if (typeof constraints.validate === 'function') {
                ...
                if (customValidationResult !== true) {
                    ...
                    return {
                        valid: false,
                        error: createUserFriendlyError(ZKErrorCode.INVALID_CIRCUIT_INPUT, errorMessage, {
                            severity: ErrorSeverity.ERROR,
                            operationId,
                            details: {
                                fieldName: inputName,
        ...
        // Handle unexpected errors
        const validationError = error.name === 'ZKError' ? error : createZKError(ZKErrorCode.CIRCUIT_INPUT_VALIDATION_FAILED, `Unexpected error validating ${inputName}: ${error.message}`, {
            severity: ErrorSeverity.ERROR,
            operationId,
            details: {
                fieldName: inputName,
